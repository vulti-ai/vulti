...
pub fn update_agent(
    ...
    // Update model in config.yaml if provided (snapshot config first)
...
/// Delete the agent's vault — removes the .vult keyshare, any vault:*.json files,
/// and clears crypto from wallet.json.
#[tauri::command]
pub fn delete_agent_vault(agent_id: String) -> Result<(), String> {
    let home = agent_home(&agent_id);
    if !home.exists() {
        return Err(format!("Agent '{}' not found", agent_id));
    }
    ...
    // Remove all .vult keyshare files
    if let Ok(entries) = fs::read_dir(&home) {
        for entry in entries.flatten() {
            let path = entry.path();
            if path.extension().and_then(|e| e.to_str()) == Some("vult") {
                if let Err(e) = fs::remove_file(&path) {
                    eprintln!("Failed to remove vault file {:?}: {}", path, e);
                } else {
    ...
    // Remove any vault:*.json files (keyshare copies from ~/.vultisig)
    if let Ok(entries) = fs::read_dir(&home) {
        for entry in entries.flatten() {
    ...
    // Clear crypto from wallet.json
    let wallet_path = home.join("wallet.json");
    if wallet_path.exists() {
        if let Ok(content) = fs::read_to_string(&wallet_path) {
            if let Ok(mut wallet) = serde_json::from_str::<WalletFile>(&content) {
                wallet.crypto = None;
    ...
    if deleted.is_empty() {
        Err("No vault files found for this agent".to_string())
    } else {
        Ok(())
    }
}
...
/// Snapshot current config.yaml to config_history/{timestamp}.yaml
fn snapshot_config(agent_id: &str) {
    let config_path = agent_home(agent_id).join("config.yaml");
    if !config_path.exists() {
        return;
    }
    let history_dir = agent_home(agent_id).join("config_history");
    let _ = ensure_dir(&history_dir);

    let timestamp = chrono::Utc::now().format("%Y%m%d_%H%M%S").to_string();
    let dest = history_dir.join(format!("{}.yaml", timestamp));
    if dest.exists() {
        return; // Already snapped this second
    }
...
#[tauri::command]
pub fn list_config_revisions(agent_id: String) -> Result<Vec<ConfigRevision>, String> {
    let history_dir = agent_home(&agent_id).join("config_history");
    ...
    if let Ok(entries) = fs::read_dir(&history_dir) {
        for entry in entries.flatten() {
            ...
            let timestamp = if stem.len() == 15 && stem.contains('_') {
            ...
            revisions.push(ConfigRevision { revision: name, timestamp, size });
        }
    }
...
#[tauri::command]
pub fn get_config_revision(agent_id: String, revision: String) -> Result<String, String> {
    let path = agent_home(&agent_id).join("config_history").join(&revision);
    if !path.exists() {
...
#[tauri::command]
pub fn rollback_config(agent_id: String, revision: String) -> Result<(), String> {
    let target = agent_home(&agent_id).join("config_history").join(&revision);
    if !target.exists() {
        return Err(format!("Revision '{}' not found", revision));
    }
    ...
    let config_path = agent_home(&agent_id).join("config.yaml");
    fs::copy(&target, &config_path).map_err(|e| format!("Rollback failed: {}", e))?;
...
fn approvals_path() -> std::path::PathBuf {
    vulti_home().join("approvals").join("pending.json")
}

fn load_approvals() -> Vec<Approval> {
...
fn save_approvals(approvals: &[Approval]) -> Result<(), String> {
    let path = approvals_path();
    if let Some(parent) = path.parent() {
        ensure_dir(parent)?;
    }
    let content = serde_json::to_string_pretty(approvals).map_err(|e| e.to_string())?;
    fs::write(&path, content).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn list_approvals(agent_id: Option<String>) -> Vec<Approval> {
    let now = chrono::Utc::now().to_rfc3339();
    load_approvals()
        .into_iter()
...
#[tauri::command]
pub fn resolve_approval(approval_id: String, approved: bool) -> Result<Approval, String> {
    let mut approvals = load_approvals();
    let now = chrono::Utc::now().to_rfc3339();

    let a = approvals
        .iter_mut()
        .find(|a| a.id == approval_id && a.status == "pending")
        .ok_or_else(|| format!("Approval '{}' not found or already resolved", approval_id))?;

    a.status = if approved { "approved" } else { "denied" }.to_string();
    a.resolved_at = Some(now);
    a.resolved_by = Some("owner".to_string());
    let result = a.clone();
