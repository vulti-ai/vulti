# OpenAI-Compatible API Server for Vulti
...
SSE streaming. By exposing this endpoint, vulti-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  vulti-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.vulti/.env`
2. Run `vulti gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with vulti-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns vulti-agent as a model) |
...
Authorization: Bearer vulti-api-key-here
...
But vulti-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "vulti-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured vulti-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of vulti-agent's core prompt
...
Once implemented, vulti-agent works as a drop-in backend for:
