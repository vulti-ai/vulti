"""ACP agent server — exposes Vulti via the Agent Client Protocol."""
...
try:
    from vulti_cli import __version__ as VULTI_VERSION
except Exception:
    VULTI_VERSION = "0.0.0"
...
class VultiACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Vulti AIAgent."""
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethod(
                    ...
                    description=f"Authenticate Vulti using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="vulti-agent", version=VULTI_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Vulti on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from vulti_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["vulti-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Vulti v{VULTI_VERSION}"
