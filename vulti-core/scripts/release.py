...
VERSION_FILE = REPO_ROOT / "vulti_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "vulti@nousresearch.com": "NousResearch",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/vulti-agent",
    ...
    lines.append(f"# Vulti v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Vulti. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Vulti Release Tool")
    ...
    if args.publish:
        ...
        git("tag", "-a", tag_name, "-m",
            f"Vulti v{new_version} ({calver_date})\n\nWeekly release")
        ...
        result = subprocess.run(
            ["gh", "release", "create", tag_name,
             "--title", f"Vulti v{new_version} ({calver_date})",
        ...
        if result.returncode == 0:
            ...
            print(f"    gh release create {tag_name} --title 'Vulti v{new_version} ({calver_date})'")
