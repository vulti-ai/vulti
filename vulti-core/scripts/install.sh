...
check_node() {
    ...
    if [ -x "$VULTI_HOME/node/bin/node" ]; then
        export PATH="$VULTI_HOME/node/bin:$PATH"
        local found_ver=$("$VULTI_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Vulti-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.vulti/node/..."
    ...
    # Place into ~/.vulti/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$VULTI_HOME/node"
    mkdir -p "$VULTI_HOME"
    mv "$extracted_dir" "$VULTI_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$VULTI_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$VULTI_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$VULTI_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$VULTI_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$VULTI_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.vulti/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Vulti itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="vulti-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Vulti behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up vulti command..."

    if [ "$USE_VENV" = true ]; then
        VULTI_BIN="$INSTALL_DIR/venv/bin/vulti"
    else
        VULTI_BIN="$(which vulti 2>/dev/null || echo "")"
        if [ -z "$VULTI_BIN" ]; then
            log_warn "vulti not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$VULTI_BIN" ]; then
        log_warn "vulti entry point not found at $VULTI_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$VULTI_BIN" "$HOME/.local/bin/vulti"
    log_success "Symlinked vulti → ~/.local/bin/vulti"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Vulti — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so vulti works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "vulti command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.vulti directory structure (config at top level, code in subdir)
    mkdir -p "$VULTI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.vulti/.env (top level, easy to find)
    if [ ! -f "$VULTI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$VULTI_HOME/.env"
            log_success "Created ~/.vulti/.env from template"
        else
            touch "$VULTI_HOME/.env"
            log_success "Created ~/.vulti/.env"
        fi
    else
        log_info "~/.vulti/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.vulti/config.yaml (top level, easy to find)
    if [ ! -f "$VULTI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$VULTI_HOME/config.yaml"
            log_success "Created ~/.vulti/config.yaml from template"
        fi
    else
        log_info "~/.vulti/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$VULTI_HOME/SOUL.md" ]; then
        cat > "$VULTI_HOME/SOUL.md" << 'SOUL_EOF'
# Vulti Persona
...
-->
SOUL_EOF
        log_success "Created ~/.vulti/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.vulti/"

    # Seed bundled skills into ~/.vulti/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.vulti/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.vulti/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$VULTI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$VULTI_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.vulti/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — Vulti itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'vulti setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m vulti_cli.main setup < /dev/tty
    else
        python -m vulti_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$VULTI_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Vulti to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$VULTI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'vulti whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                VULTI_CMD="$HOME/.local/bin/vulti"
                [ ! -x "$VULTI_CMD" ] && VULTI_CMD="vulti"
                $VULTI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'vulti whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'vulti gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $VULTI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $VULTI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: vulti gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: vulti gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $VULTI_CMD gateway > "$VULTI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.vulti/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: vulti gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: vulti gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.vulti/):${NC}"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}vulti${NC}              Start chatting"
    echo -e "   ${GREEN}vulti setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}vulti config${NC}       View/edit configuration"
    echo -e "   ${GREEN}vulti config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}vulti gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}vulti update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'vulti' command:${NC}"
    echo ""
