 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.vulti/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.vulti', 'whatsapp', 'session'));
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Vulti' own reply messages in self-chat mode to avoid loops.
      if (msg.key.fromMe && (body.startsWith('🛡 *Vulti*') || recentlySentIds.has(msg.key.id))) {
        if (WHATSAPP_DEBUG) {
