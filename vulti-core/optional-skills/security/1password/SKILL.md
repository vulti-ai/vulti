...
version: 1.0.0
author: arceus77-7, enhanced by Vulti
license: MIT
metadata:
  vulti:
...
- `tmux` available for stable authenticated sessions during Vulti terminal calls (desktop app flow only)
...
### Service Account (recommended for Vulti)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.vulti/.env` (the skill will prompt for this on first load).
...
## Vulti Execution Pattern (desktop app flow)

Vulti terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/vulti-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/vulti-op.sock"
