...
description: Migrate a user's OpenClaw customization footprint into Vulti. Imports Vulti-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Vulti (Nous Research)
license: MIT
metadata:
  vulti:
    tags: [Migration, OpenClaw, Vulti, Memory, Persona, Import]
    related_skills: [vulti-agent]
---

# OpenClaw -> Vulti Migration

Use this skill when a user wants to move their OpenClaw setup into Vulti with minimal manual cleanup.
...
```bash
vulti claw migrate              # Full interactive migration
...
vulti claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `vulti setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_vulti.py` to:

- import `SOUL.md` into the Vulti home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Vulti memory entries
- merge OpenClaw command approval patterns into Vulti `command_allowlist`
- migrate Vulti-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.vulti/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Vulti workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.vulti/tts/`
- archive non-secret docs that do not have a direct Vulti destination
...
- `scripts/openclaw_to_vulti.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py`

Do not guess a shorter path like `~/.vulti/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.vulti/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Vulti-compatible secrets, currently:
...
Vulti CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Vulti. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Vulti workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Vulti skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.vulti/skills/migration/openclaw-migration/scripts/openclaw_to_vulti.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Vulti unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Vulti targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Vulti destination. Unsupported auth blobs must still be reported as skipped.
...
- Vulti persona state imported
- Vulti memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.vulti/skills/openclaw-imports/`
