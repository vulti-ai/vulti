"""Anthropic Messages API adapter for Vulti.

Translates between Vulti's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Vulti historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Vulti)
    ...
    # 1. Vulti-managed OAuth/setup token env var
    ...
    # 3. Vulti-managed OAuth credentials (~/.vulti/.anthropic_oauth.json)
    vulti_creds = read_vulti_oauth_credentials()
    if vulti_creds:
        if is_claude_code_token_valid(vulti_creds):
            logger.debug("Using Vulti-managed OAuth credentials")
            return vulti_creds["accessToken"]
        # Expired — try refresh
        logger.debug("Vulti OAuth token expired — attempting refresh")
        refreshed = refresh_vulti_oauth_token()
        if refreshed:
            return refreshed
    ...
    # This remains as a compatibility fallback for pre-migration Vulti configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── Vulti-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.vulti/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_VULTI_OAUTH_FILE = Path(os.getenv("VULTI_HOME", str(Path.home() / ".vulti"))) / ".anthropic_oauth.json"
...
def run_vulti_oauth_login() -> Optional[str]:
    """Run Vulti-native OAuth PKCE flow for Claude Pro/Max subscription.

    Opens a browser to claude.ai for authorization, prompts for the code,
    exchanges it for tokens, and stores them in ~/.vulti/.anthropic_oauth.json.
    ...
    print()
    print("Authorize Vulti with your Claude Pro/Max subscription.")
    print()
    ...
    _save_vulti_oauth_credentials(access_token, refresh_token, expires_at_ms)
...
def _save_vulti_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.vulti/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        _VULTI_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
        _VULTI_OAUTH_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
        _VULTI_OAUTH_FILE.chmod(0o600)
    except (OSError, IOError) as e:
        logger.debug("Failed to save Vulti OAuth credentials: %s", e)


def read_vulti_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Vulti-managed OAuth credentials from ~/.vulti/.anthropic_oauth.json."""
    if _VULTI_OAUTH_FILE.exists():
        try:
            data = json.loads(_VULTI_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Vulti OAuth credentials: %s", e)
    return None


def refresh_vulti_oauth_token() -> Optional[str]:
    """Refresh the Vulti-managed OAuth token using the stored refresh token.
    ...
    creds = read_vulti_oauth_credentials()
    ...
    try:
        ...
        if new_access:
            new_expires_ms = int(time.time() * 1000) + (expires_in * 1000)
            _save_vulti_oauth_credentials(new_access, new_refresh, new_expires_ms)
            # Also update Claude Code's credential file
            _write_claude_code_credentials(new_access, new_refresh, new_expires_ms)
            logger.debug("Successfully refreshed Vulti OAuth token")
            return new_access
    except Exception as e:
        logger.debug("Failed to refresh Vulti OAuth token: %s", e)
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Vulti", "Claude Code")
                text = text.replace("Vulti", "Claude Code")
                text = text.replace("vulti-agent", "claude-code")
