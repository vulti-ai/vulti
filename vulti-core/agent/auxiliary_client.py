...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.vulti/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=vulti-agent"]}
...
_AUTH_JSON_PATH = get_vulti_home() / "auth.json"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.vulti/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid Codex OAuth access token from Vulti auth store (~/.vulti/auth.json)."""
    try:
        from vulti_cli.auth import _read_codex_tokens
...
def _read_main_model() -> str:
    ...
    Falls back through VULTI_MODEL → LLM_MODEL → config.yaml model.default
    so the auxiliary client can use the same model as the main agent when no
    dedicated auxiliary model is available.
    """
    from_env = os.getenv("OPENAI_MODEL") or os.getenv("VULTI_MODEL") or os.getenv("LLM_MODEL")
    if from_env:
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "nous":
        ...
        if client is None:
            logger.warning("auxiliary.provider=nous but Nous Portal not configured (run: vulti login)")
        return client, model

    if forced == "codex":
        client, model = _try_codex()
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (run: vulti model)")
        return client, model
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: vulti login)")
            return None, None
        final_model = model or default
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: vulti model)")
                return None, None
    ...
    try:
        from vulti_cli.auth import PROVIDER_REGISTRY, _resolve_kimi_base_url
    except ImportError:
        logger.debug("vulti_cli.auth not available for provider %s", provider)
        return None, None
...
def get_available_vision_backends() -> List[str]:
    ...
    it is also a known-good vision backend; otherwise Vulti falls back through
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=vulti-agent"])
    if merged_extra:
