...
being persisted to ~/.vulti/discord_threads.json.
"""
...
class TestDiscordThreadPersistence:
    ...
    def _make_adapter(self, tmp_path):
        """Build a minimal DiscordAdapter with VULTI_HOME pointed at tmp_path."""
    ...
    def test_missing_vulti_home_does_not_crash(self, tmp_path):
        """Load/save tolerate missing directories."""
        fake_home = tmp_path / "nonexistent" / "deep"
        with patch.dict(os.environ, {"VULTI_HOME": str(fake_home)}):
            from gateway.platforms.discord import DiscordAdapter
