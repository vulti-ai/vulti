...
class TestReasoningCommand:
    ...
    async def test_reasoning_command_reloads_current_state_from_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_vulti_home", vulti_home)
        monkeypatch.delenv("VULTI_REASONING_EFFORT", raising=False)
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        vulti_home = tmp_path / "vulti"
        vulti_home.mkdir()
        (vulti_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")

        monkeypatch.setattr(gateway_run, "_vulti_home", vulti_home)
        monkeypatch.setattr(gateway_run, "_env_path", vulti_home / ".env")
    ...
    def test_run_agent_prefers_config_over_stale_reasoning_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("VULTI_REASONING_EFFORT", "low")
        fake_run_agent = types.ModuleType("run_agent")
        fake_run_agent.AIAgent = _CapturingAgent
        monkeypatch.setitem(sys.modules, "run_agent", fake_run_agent)
