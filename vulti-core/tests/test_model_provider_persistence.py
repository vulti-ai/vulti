"""Tests that provider selection via `vulti model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated VULTI_HOME with a minimal string-format config."""
    home = tmp_path / "vulti"
    home.mkdir()
    ...
    monkeypatch.delenv("VULTI_MODEL", raising=False)
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.delenv("VULTI_INFERENCE_PROVIDER", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from vulti_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from vulti_cli.main import _model_flow_api_key_provider
        ...
        with patch("vulti_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("vulti_cli.auth.deactivate_provider"), \
