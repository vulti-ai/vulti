"""Tests for acp_adapter.server — VultiACPAgent ACP server."""
...
from acp_adapter.server import VultiACPAgent, VULTI_VERSION
...
def agent(mock_manager):
    """VultiACPAgent backed by a mock session manager."""
    return VultiACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "vulti-agent"
        assert resp.agent_info.version == VULTI_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert VULTI_VERSION in result
