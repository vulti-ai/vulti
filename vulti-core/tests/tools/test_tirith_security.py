...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._vulti_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_vulti_bin(self):
        """After _INSTALL_FAILED, manual install in VULTI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        vulti_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(vulti_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(vulti_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._vulti_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == vulti_bin
            assert _tirith_mod._resolved_path == vulti_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# VULTI_HOME isolation
# ---------------------------------------------------------------------------

class TestVultiHomeIsolation:
    def test_vulti_bin_dir_respects_vulti_home(self):
        """_vulti_bin_dir must use VULTI_HOME, not hardcoded ~/.vulti."""
        from tools.tirith_security import _vulti_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"VULTI_HOME": tmpdir}):
            result = _vulti_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_vulti_home(self):
        """_failure_marker_path must use VULTI_HOME, not hardcoded ~/.vulti."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"VULTI_HOME": "/custom/vulti"}):
            result = _failure_marker_path()
        assert result == "/custom/vulti/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets VULTI_HOME; verify it's active."""
        vulti_home = os.getenv("VULTI_HOME")
        assert vulti_home is not None, "VULTI_HOME should be set by conftest"
        assert "vulti_test" in vulti_home, "Should point to test temp dir"

    def test_get_vulti_home_fallback(self):
        """Without VULTI_HOME set, falls back to ~/.vulti."""
        from tools.tirith_security import _get_vulti_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove VULTI_HOME entirely
            os.environ.pop("VULTI_HOME", None)
            result = _get_vulti_home()
        assert result == os.path.join(os.path.expanduser("~"), ".vulti")
