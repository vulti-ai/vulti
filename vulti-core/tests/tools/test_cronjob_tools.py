...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because vulti uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        monkeypatch.delenv("VULTI_INTERACTIVE", raising=False)
        monkeypatch.setenv("VULTI_GATEWAY_SESSION", "1")
        monkeypatch.delenv("VULTI_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("VULTI_INTERACTIVE", raising=False)
        monkeypatch.delenv("VULTI_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("VULTI_EXEC_ASK", "1")
...
class TestScheduleCronjob:
    ...
    def test_thread_id_captured_in_origin(self, monkeypatch):
        ...
        monkeypatch.setenv("VULTI_SESSION_CHAT_ID", "123456")
        monkeypatch.setenv("VULTI_SESSION_THREAD_ID", "42")
