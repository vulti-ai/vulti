...
def _clean_state():
    ...
    key = os.getenv("VULTI_SESSION_KEY", "default")
    clear_session(key)
    approval_module._permanent_approved.clear()
    saved = {}
    for k in ("VULTI_INTERACTIVE", "VULTI_GATEWAY_SESSION", "VULTI_EXEC_ASK", "VULTI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithAllowDangerous:
    ...
    def test_dangerous_only_gateway(self, mock_tirith):
        os.environ["VULTI_GATEWAY_SESSION"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No VULTI_INTERACTIVE or VULTI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
