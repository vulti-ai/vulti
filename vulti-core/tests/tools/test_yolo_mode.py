"""Tests for --yolo (VULTI_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When VULTI_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("VULTI_INTERACTIVE", "1")
        monkeypatch.setenv("VULTI_SESSION_KEY", "test-session")
        monkeypatch.delenv("VULTI_YOLO_MODE", raising=False)
        monkeypatch.delenv("VULTI_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("VULTI_EXEC_ASK", raising=False)
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With VULTI_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("VULTI_YOLO_MODE", "1")
        monkeypatch.setenv("VULTI_INTERACTIVE", "1")
        monkeypatch.setenv("VULTI_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """VULTI_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("VULTI_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for VULTI_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("VULTI_YOLO_MODE", "")
        monkeypatch.setenv("VULTI_INTERACTIVE", "1")
        monkeypatch.setenv("VULTI_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("VULTI_YOLO_MODE") returns ""
