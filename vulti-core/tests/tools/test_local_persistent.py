...
class TestLocalOneShotRegression:
    ...
    def test_state_does_not_persist(self):
        env = LocalEnvironment(persistent=False)
        env.execute("export VULTI_ONESHOT_LOCAL=yes")
        r = env.execute("echo $VULTI_ONESHOT_LOCAL")
...
class TestLocalPersistent:
    ...
    def test_env_var_persists(self, env):
        env.execute("export VULTI_LOCAL_PERSIST_TEST=works")
        r = env.execute("echo $VULTI_LOCAL_PERSIST_TEST")
