from vulti_cli import runtime_provider as rp
...
def test_custom_endpoint_auto_provider_prefers_openai_key(monkeypatch):
    ...
    Same as #560 but via 'vulti model' flow which sets provider to 'auto'.
    """
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openrouter")
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
...
def test_resolve_requested_provider_precedence(monkeypatch):
    monkeypatch.setenv("VULTI_INFERENCE_PROVIDER", "nous")
