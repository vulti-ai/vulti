...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (vulti_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
...
class TestGetTextAuxiliaryClient:
    ...
    def test_custom_endpoint_uses_config_saved_base_url(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENAI_API_KEY", "lm-studio-key")
        monkeypatch.setattr("vulti_cli.config.load_config", lambda: config)
        monkeypatch.setattr("vulti_cli.runtime_provider.load_config", lambda: config)
...
class TestVisionClientFallback:
    ...
    def test_selected_anthropic_provider_is_preferred_for_vision_auto(self, monkeypatch):
        ...
        with (
            ...
            patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api03-key"),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("vulti_cli.config.load_config", fake_load_config),
        ):
            client, model = get_vision_auxiliary_client()
...
class TestTaskSpecificOverrides:
    ...
    def test_task_direct_endpoint_from_config(self, monkeypatch, tmp_path):
        vulti_home = tmp_path / "vulti"
        vulti_home.mkdir(parents=True, exist_ok=True)
        (vulti_home / "config.yaml").write_text(
            """auxiliary:
  web_extract:
