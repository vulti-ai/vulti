...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from vulti_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        vulti_home = tmp_path / ".vulti"
        vulti_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: vulti_home / ".env")
        monkeypatch.setattr(config_module, "get_vulti_home", lambda: vulti_home)
        from vulti_cli.main import _has_any_provider_configured
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from vulti_cli.main import _PROVIDER_MODELS
