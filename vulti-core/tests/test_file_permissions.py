...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("vulti_cli.config.get_config_path", return_value=config_path), \
             patch("vulti_cli.config.ensure_vulti_home"):
            from vulti_cli.config import save_config
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("vulti_cli.config.get_env_path", return_value=env_path), \
             patch("vulti_cli.config.ensure_vulti_home"):
    ...
    def test_ensure_vulti_home_sets_0700(self):
        home = Path(self.tmpdir) / ".vulti"
        with patch("vulti_cli.config.get_vulti_home", return_value=home):
