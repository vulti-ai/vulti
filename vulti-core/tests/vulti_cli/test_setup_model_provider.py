...
from vulti_cli.config import load_config, save_config, save_env_value
from vulti_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_keep_current_custom_from_config_does_not_fall_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vulti_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("vulti_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("vulti_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("vulti_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("vulti_cli.auth.detect_external_credentials", lambda: [])
...
def test_setup_custom_endpoint_saves_working_v1_base_url(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vulti_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("vulti_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("vulti_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("vulti_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("vulti_cli.auth.detect_external_credentials", lambda: [])
...
def test_setup_keep_current_config_provider_uses_provider_specific_model_menu(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vulti_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("vulti_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("vulti_cli.models.provider_model_ids", lambda provider: [])
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_keep_current_anthropic_can_configure_openai_vision_default(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vulti_cli.setup.prompt_choice", lambda *args, **kwargs: next(picks))
    monkeypatch.setattr(
        "vulti_cli.setup.prompt",
...
def test_setup_switch_custom_to_codex_clears_custom_endpoint_and_updates_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vulti_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("vulti_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("vulti_cli.auth._login_openai_codex", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "vulti_cli.auth.resolve_codex_runtime_credentials",
        lambda *args, **kwargs: {
...
def test_setup_summary_marks_codex_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'vulti setup' to configure" not in output
