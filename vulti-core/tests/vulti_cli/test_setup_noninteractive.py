...
class TestNonInteractiveSetup:
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("vulti_cli.setup.ensure_vulti_home"),
            patch("vulti_cli.setup.load_config", return_value={}),
            patch("vulti_cli.setup.get_vulti_home", return_value="/tmp/.vulti"),
            patch("vulti_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "vulti config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `vulti` should not prompt for input when no provider exists and stdin is headless."""
        from vulti_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("vulti_cli.main._has_any_provider_configured", return_value=False),
            patch("vulti_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
