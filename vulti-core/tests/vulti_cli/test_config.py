"""Tests for vulti_cli configuration management."""
...
class TestGetVultiHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("VULTI_HOME", None)
            home = get_vulti_home()
            assert home == Path.home() / ".vulti"

    def test_env_override(self):
        with patch.dict(os.environ, {"VULTI_HOME": "/custom/path"}):
            home = get_vulti_home()
            assert home == Path("/custom/path")


class TestEnsureVultiHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"VULTI_HOME": str(tmp_path)}, clear=False):
