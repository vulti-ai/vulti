"""Tests for vulti_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from vulti_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("vulti_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from vulti_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("vulti_cli.config.load_config")
    ...
    @patch("vulti_cli.config.load_config")
    @patch.dict("os.environ", {"VULTI_PLATFORM": "discord"})
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from vulti_cli.skills_config import _get_categories
        skills = [
