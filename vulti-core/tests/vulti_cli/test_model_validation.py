"""Tests for provider-aware `/model` validation in vulti_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("vulti_cli.models.fetch_api_models", return_value=api_models), \
         patch("vulti_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:vulti-3", "openrouter")
        assert provider == "nous"
        assert model == "vulti-3"
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("vulti_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("vulti_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
