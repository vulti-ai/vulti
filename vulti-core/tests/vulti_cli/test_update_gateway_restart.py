...
Ensures ``vulti update`` correctly detects running gateways managed by
...
import vulti_cli.gateway as gateway_cli
from vulti_cli.main import cmd_update
...
class TestLaunchdPlistRefresh:
    ...
    def test_refresh_rewrites_stale_plist(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.vulti.gateway.plist"
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: vulti gateway run'."""
        ...
        assert "Restart it with: vulti gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart it with: vulti gateway run" in captured
