...
import vulti_cli.doctor as doctor
import vulti_cli.gateway as gateway_cli
from vulti_cli import doctor as doctor_mod
from vulti_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "VULTI_HOME", vulti_home)
    monkeypatch.delenv("VULTI_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("VULTI_INTERACTIVE")
        raise SystemExit(0)
