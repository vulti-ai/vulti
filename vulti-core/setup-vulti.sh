...
# 5. Symlinks the 'vulti' CLI command into ~/.local/bin
...
# ============================================================================
# PATH setup — symlink vulti into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up vulti command..."

VULTI_BIN="$SCRIPT_DIR/venv/bin/vulti"
mkdir -p "$HOME/.local/bin"
ln -sf "$VULTI_BIN" "$HOME/.local/bin/vulti"
echo -e "${GREEN}✓${NC} Symlinked vulti → ~/.local/bin/vulti"
...
# ============================================================================
# Seed bundled skills into ~/.vulti/skills/
# ============================================================================

VULTI_SKILLS_DIR="${VULTI_HOME:-$HOME/.vulti}/skills"
mkdir -p "$VULTI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.vulti/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$VULTI_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  vulti status        # Check configuration"
echo "  vulti gateway install # Install gateway service (messaging + cron)"
echo "  vulti cron list     # View scheduled jobs"
echo "  vulti doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m vulti_cli.main setup
fi
