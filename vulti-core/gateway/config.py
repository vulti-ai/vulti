...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_vulti_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_vulti_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.vulti/gateway.json
    3. cli-config.yaml gateway section
    4. Defaults
    """
    config = GatewayConfig()
    
    # Try loading from ~/.vulti/gateway.json
    ...
    # for session reset policy since that's where vulti setup writes it.
    try:
        import yaml
...
def _get_or_create_web_token() -> str:
    ...
    Stored in ~/.vulti/web_token so it persists across restarts.
    """
    import secrets
    token_path = get_vulti_home() / "web_token"
...
def save_gateway_config(config: GatewayConfig) -> None:
    """Save gateway configuration to ~/.vulti/gateway.json."""
    gateway_config_path = get_vulti_home() / "gateway.json"
