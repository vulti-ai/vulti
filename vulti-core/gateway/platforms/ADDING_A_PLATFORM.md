...
Checklist for integrating a new messaging platform into the Vulti gateway.
...
And add it to the `vulti-gateway` composite:

```python
"vulti-gateway": {
    "includes": [..., "vulti-your-platform"]
}
```
...
## 12. Status Display (`vulti_cli/status.py`)
...
## 13. Gateway Setup Wizard (`vulti_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ vulti_cli/ toolsets.py \
  --include="*.py" -l | sort -u
