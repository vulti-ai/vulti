...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.vulti/.env manually."
)
...
# Default location: {VULTI_HOME}/image_cache/
IMAGE_CACHE_DIR = get_vulti_home() / "image_cache"
...
async def cache_image_from_url(url: str, ext: str = ".jpg") -> str:
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
        response = await client.get(
            url,
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; VultiAgent/1.0)",
                "Accept": "image/*,*/*;q=0.8",
            },
        )
...
AUDIO_CACHE_DIR = get_vulti_home() / "audio_cache"
...
DOCUMENT_CACHE_DIR = get_vulti_home() / "document_cache"
...
class BasePlatformAdapter(ABC):
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          VULTI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          VULTI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          VULTI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        import random

        mode = os.getenv("VULTI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("VULTI_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("VULTI_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
