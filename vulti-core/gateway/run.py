...
# Load environment variables from ~/.vulti/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from vulti_cli.env_loader import load_vulti_dotenv
_env_path = _vulti_home / '.env'
load_vulti_dotenv(vulti_home=_vulti_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _vulti_home / 'config.yaml'
    ...
    try:
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["VULTI_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → VULTI_TIMEZONE env var.
        # VULTI_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "VULTI_TIMEZONE" not in os.environ:
            os.environ["VULTI_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["VULTI_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["VULTI_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["VULTI_EXEC_ASK"] = "1"
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("VULTI_INFERENCE_PROVIDER"),
        )
...
def _resolve_vulti_bin() -> Optional[list[str]]:
    """Resolve the Vulti update command as argv parts.

    Tries in order:
    1. ``shutil.which("vulti")`` — standard PATH lookup
    2. ``sys.executable -m vulti_cli.main`` — fallback when Vulti is running
       from a venv/module invocation and the ``vulti`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("vulti_cli") is not None:
            return [sys.executable, "-m", "vulti_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    _VOICE_MODE_PATH = _vulti_home / "gateway_voice_mode.json"
    ...
    def _build_onboard_connections_context(self, agent_id: str) -> str:
        ...
        try:
            ...
            return (
                ...
                "When the user asks you to connect to a service or approves a connection, "
                "call manage_own_connections(action='add', connection_names=['...']) to enable it yourself. "
                "Do NOT tell the user to run CLI commands — you have the tool to do it directly.\n"
                "If they deny a connection, respect that.\n\n"
            )
        except Exception:
            return ""
    ...
    def _build_onboard_connections_prompt(self, agent_name: str, soul_path, agent_id: str) -> str:
        ...
        return (
            ...
            "Your goal: Choose which connections YOU need to do your job.\n\n"
            f"{connections_block}"
            f"{env_block}"
            f"{skills_block}"
            "WORKFLOW — follow these steps exactly:\n"
            "1. Based on your role, identify the 3-5 connections you actually NEED.\n"
            "2. STOP and present your suggestions to the user as a clear list:\n"
            "   'Based on my role, I'd like access to these connections:'\n"
            "   - connection_name — reason why you need it\n"
            "   Then ask: 'Shall I enable these?'\n"
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks VULTI_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.vulti/config.yaml.
        Relative paths are resolved from ~/.vulti/.
        """
        import json as _json
        file_path = os.getenv("VULTI_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _vulti_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks VULTI_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.vulti/config.yaml.
        """
        prompt = os.getenv("VULTI_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        VULTI_REASONING_EFFORT as a fallback. Valid: "xhigh", "high",
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("VULTI_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _vulti_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Vulti Gateway...")
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.vulti/.env to allow open access, "
        ...
        if not notified and any(
            path.exists()
            for path in (
                _vulti_home / ".update_pending.json",
                _vulti_home / ".update_pending.claimed.json",
            )
        ):
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'vulti-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`vulti pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        # in vulti_cli/commands.py — no hardcoded set to maintain here.
        from vulti_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if history and len(history) >= 4:
            ...
            try:
                _hyg_cfg_path = _vulti_home / "config.yaml"
            ...
            if _areg:
                ...
                _HUB_CHANNEL_PROMPTS = {
                    "onboard": (
                        ...
                        + "YOUR JOB: Have a natural conversation to discover your identity.\n\n"
                        "The home dashboard updates automatically from API data — do NOT use modify_pane for home widgets.\n"
                        "Use modify_pane only to show results in the chat pane during conversation.\n\n"
                        ...
                        "3. SKILLS — Once you know the role, IMMEDIATELY install relevant skills using skills_list() "
                        "to find them and skill_manage(action='install') to install them. Do NOT just mention skills — "
                        "install them right away. Tell the user what you installed.\n"
                        "4. CONNECTIONS — Which of the available connections do I need? (see list above) "
                        "Present your suggestions as a clear list. Then STOP. Do NOT ask any follow-up questions. "
                        "Do NOT continue to step 5 or 6. WAIT for the user to enable the connections before proceeding.\n"
                        ...
                        "Save your knowledge to files as you go:\n"
                        ...
                        "5. Call update_own_profile(role='the_role') whenever you set or change your role.\n\n"
                        "When you feel the setup is complete, tell the user they're all set.]"
                    ),
                    "home": (
                        f"[System: The user is viewing the home dashboard for agent '{_aname}'. "
                        "The home dashboard shows live widgets that update automatically from API data. "
                        "You cannot modify home widgets — they reflect the agent's actual state. "
                        "Help the user with any questions about what they see, or assist with changes "
                        "to the underlying data (connections, skills, cron jobs, rules, etc.).]"
                    ),
                    "profile": (
                    ...
                    "onboard-role": (
                        ...
                        "  3. USER.md (at least basics)\n"
                        "  4. MEMORY.md (at least 1-2 facts)\n"
                        "If any are missing, write them before saying Next.]"
                    ),
                    ...
                    "onboard-skills": (
                        ...
                        "WORKFLOW — follow these steps exactly:\n"
                        "1. List available skills and identify 3-5 that match your role.\n"
                        "2. IMMEDIATELY install them — do NOT wait for user confirmation. Skills are free to install.\n"
                        "3. Tell the user what you installed and why.\n"
                        "4. If the user wants to add or remove any, do so.\n\n"
                        "For example:\n"
                        ...
                        "The home dashboard updates automatically — do NOT use modify_pane for home widgets.\n"
                    ...
                    "introspect": (
                        ...
                        "The home dashboard updates automatically from API data — do NOT use modify_pane for home widgets.\n"
                        "Use modify_pane only for chat-session widgets (showing results, tables, status to the user during conversation).\n"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Vulti delivers cron job results "
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from vulti_cli.commands import gateway_help_lines
        lines = [
            "📖 **Vulti Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> str:
        ...
        if current_provider == "auto":
            try:
                from vulti_cli.auth import resolve_provider as _resolve_provider
        ...
        if target_provider == current_provider:
            from vulti_cli.models import detect_provider_for_model
        ...
        os.environ["VULTI_MODEL"] = new_model
        if provider_changed:
            os.environ["VULTI_INFERENCE_PROVIDER"] = target_provider
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Setup: `vulti setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.vulti/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Vulti with the messaging extra, e.g. "
                    "`pip install vulti-agent[messaging]`."
                )
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            audio_path = os.path.join(
                tempfile.gettempdir(), "vulti_voice",
    ...
    async def _run_background_task(
        ...
        try:
            ...
            default_toolset_map = {
                Platform.LOCAL: "vulti-cli",
                Platform.TELEGRAM: "vulti-telegram",
                Platform.DISCORD: "vulti-discord",
                Platform.WHATSAPP: "vulti-whatsapp",
                Platform.SLACK: "vulti-slack",
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Vulti to the latest version.

        Spawns ``vulti update`` in a separate systemd scope so it survives the
        gateway restart that ``vulti update`` may trigger at the end. Marker
        ...
        vulti_cmd = _resolve_vulti_bin()
        if not vulti_cmd:
            return (
                "✗ Could not locate the `vulti` command. "
                "Vulti is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `vulti update` manually in your terminal."
            )

        pending_path = _vulti_home / ".update_pending.json"
        output_path = _vulti_home / ".update_output.txt"
        exit_code_path = _vulti_home / ".update_exit_code"
        pending = {
        ...
        # Spawn `vulti update` in a separate cgroup so it survives gateway
        # restart. systemd-run --user --scope creates a transient scope unit.
        vulti_cmd_str = " ".join(shlex.quote(part) for part in vulti_cmd)
        update_cmd = (
            f"{vulti_cmd_str} update > {shlex.quote(str(output_path))} 2>&1; "
        ...
        self._schedule_update_notification_watch()
        return "🛡 Starting Vulti update… I'll notify you when it's done."
    ...
    async def _watch_for_update_completion(
        ...
        """Wait for ``vulti update`` to finish, then send its notification."""
        pending_path = _vulti_home / ".update_pending.json"
        claimed_path = _vulti_home / ".update_pending.claimed.json"
        exit_code_path = _vulti_home / ".update_exit_code"
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Vulti update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Vulti update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Vulti update finished successfully."
                    else:
                        msg = "❌ Vulti update failed. Check the gateway logs or run `vulti update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["VULTI_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["VULTI_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["VULTI_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["VULTI_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["VULTI_SESSION_PLATFORM", "VULTI_SESSION_CHAT_ID", "VULTI_SESSION_CHAT_NAME", "VULTI_SESSION_THREAD_ID"]:
    ...
    async def _run_agent(
        ...
        _progress_cfg = {}
        try:
            _tp_cfg_path = _vulti_home / "config.yaml"
        ...
        progress_mode = (
            ...
            or os.getenv("VULTI_TOOL_PROGRESS_MODE")
            or "all"
        )
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `vulti cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False) -> bool:
    ...
    # Prevent two gateways from running under the same VULTI_HOME.
    # The PID file is scoped to VULTI_HOME, so future multi-profile
    # setups (each profile using a distinct VULTI_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            vulti_home = os.getenv("VULTI_HOME", "~/.vulti")
            logger.error(
                "Another gateway instance is already running (PID %d, VULTI_HOME=%s). "
                "Use 'vulti gateway restart' to replace it, or 'vulti gateway stop' first.",
                existing_pid, vulti_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'vulti gateway restart' to replace it,\n"
                f"   or 'vulti gateway stop' to kill it first.\n"
                f"   Or use 'vulti gateway run --replace' to auto-replace.\n"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Vulti Gateway - Multi-platform messaging")
