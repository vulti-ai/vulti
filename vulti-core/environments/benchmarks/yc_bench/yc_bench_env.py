...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs VultiAgentLoop with terminal tool only
...
Requires: pip install vulti-agent[yc-bench]
"""
...
class YCBenchEvalConfig(VultiAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends VultiAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(VultiAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The VultiAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs VultiAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our VultiAgentLoop.
