# Vulti-Agent Atropos Environments

This directory contains the integration layer between **vulti-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  VultiAgentBaseEnv    │  vulti_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     VultiSweEnv    TerminalBench2EvalEnv
...
**VultiAgentBaseEnv** (`vulti_base_env.py`) extends BaseEnv with vulti-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves vulti-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `VultiAgentBaseEnv` and implement:
...
`VultiAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as vulti-agent's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** vulti-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any vulti-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some vulti-agent tools use `asyncio.run()` internally (e.g., mini-swe-agent's Modal backend via SWE-ReX). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Applied automatically at import time by `vulti_base_env.py`.
...
Available parsers:
- `vulti` -- Vulti/ChatML `<tool_call>` XML format
...
├── vulti_base_env.py            # Abstract base (VultiAgentBaseEnv)
...
│
├── vulti_swe_env/               # SWE-bench style training environment
│   └── vulti_swe_env.py
│
...
2. Create your env file inheriting from `VultiAgentBaseEnv`
...
2. Inherit from `VultiAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which vulti toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `vulti` |
