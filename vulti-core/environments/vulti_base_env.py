"""
VultiAgentBaseEnv -- Abstract Base Environment for Vulti-Agent + Atropos

Provides the Atropos integration plumbing that all vulti-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via VultiAgentLoop
...
# Ensure the vulti-agent repo root is on sys.path so that imports like
...
# Load API keys from vulti-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, VultiAgentLoop
from environments.tool_context import ToolContext

# Import vulti-agent toolset infrastructure
...
class VultiAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for vulti-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of vulti toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: vulti, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class VultiAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for vulti-agent Atropos integration.
    ...
    name: Optional[str] = "vulti-agent"
    env_config_cls = VultiAgentEnvConfig

    def __init__(
        self,
        config: VultiAgentEnvConfig,
        ...
        # Set terminal environment variables so vulti tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY vulti-agent tool (terminal, file, web,
