"""
VultiSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser vulti
    run-api
    python environments/vulti_swe_env.py serve \\
...
class VultiSweEnv(VultiAgentBaseEnv):
    ...
    name = "vulti-swe"
    env_config_cls = VultiSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[VultiSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = VultiSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepVulti-3-Llama-3-3B-Preview",
            tool_call_parser="vulti",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="vulti-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepVulti-3-Llama-3-3B-Preview",
