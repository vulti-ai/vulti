...
Vulti currently supports three API execution modes:
...
Vulti wraps API requests so they can be interrupted from the CLI or gateway.
...
Vulti uses two execution strategies:
...
Vulti tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, Vulti may:
