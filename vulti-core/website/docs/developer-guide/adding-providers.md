...
description: "How to add a new inference provider to Vulti — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Vulti can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `vulti model` menu entries
...
1. `vulti_cli/auth.py` decides how credentials are found.
2. `vulti_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `vulti_cli/models.py`, `vulti_cli/main.py`, and `vulti_cli/setup.py` make the provider show up in the CLI.
...
1. `vulti_cli/auth.py`
2. `vulti_cli/models.py`
3. `vulti_cli/runtime_provider.py`
4. `vulti_cli/main.py`
5. `vulti_cli/setup.py`
...
- `PROVIDER_REGISTRY` in `vulti_cli/auth.py`
- `_PROVIDER_LABELS` in `vulti_cli/models.py`
- `_PROVIDER_ALIASES` in both `vulti_cli/auth.py` and `vulti_cli/models.py`
- CLI `--provider` choices in `vulti_cli/main.py`
...
## Step 2: Add auth metadata in `vulti_cli/auth.py`
...
- What env vars should Vulti check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `vulti_cli/models.py`
...
## Step 4: Resolve runtime data in `vulti_cli/runtime_provider.py`
...
Be careful with API-key precedence. Vulti already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `vulti_cli/main.py` and `vulti_cli/setup.py`
...
If you only update one of these files, `vulti model` and `vulti setup` will drift.
...
When you add a native provider, double-check that Vulti is only sending fields that provider actually understands.
...
```bash
source .venv/bin/activate
python -m vulti_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `vulti_cli/auth.py`
- [ ] aliases added in `vulti_cli/auth.py` and `vulti_cli/models.py`
- [ ] model catalog added in `vulti_cli/models.py`
- [ ] runtime branch added in `vulti_cli/runtime_provider.py`
- [ ] CLI wiring added in `vulti_cli/main.py`
- [ ] setup wiring added in `vulti_cli/setup.py`
...
### 7. Updating `vulti model` but not `vulti setup`
