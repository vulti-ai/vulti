...
The ACP adapter wraps Vulti' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
vulti acp / vulti-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.vulti/.env
  -> configure stderr logging
  -> construct VultiACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Vulti `once`
- `allow_always` -> Vulti `always`
- reject options -> Vulti `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Vulti tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["vulti-acp"])
...
Instead it reuses Vulti' runtime resolver:

- `acp_adapter/auth.py`
- `vulti_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Vulti provider/credentials.
...
- `toolsets.py` — `vulti-acp` toolset definition
- `vulti_cli/main.py` — `vulti acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `vulti-acp` script
