...
description: "How Vulti stores sessions in SQLite, maintains lineage, and exposes recall/search"
---

# Session Storage

Vulti uses a SQLite-backed session store as the main source of truth for historical conversation state.
...
When Vulti compresses a conversation, it can continue in a new session ID while preserving ancestry via `parent_session_id`.
