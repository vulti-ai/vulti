...
description: "Complete reference of all environment variables used by Vulti"
---

# Environment Variables Reference

All variables go in `~/.vulti/.env`. You can also set them with `vulti config set VAR value`.
...
| `VULTI_MODEL` | Preferred model name (checked before `LLM_MODEL`, used by gateway) |
| `LLM_MODEL` | Default model name (fallback when not set in config.yaml) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `VULTI_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `VULTI_LOCAL_STT_LANGUAGE` | Default language passed to `VULTI_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `VULTI_HOME` | Override Vulti config directory (default: `~/.vulti`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Vulti prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.

| Variable | Description |
|----------|-------------|
| `VULTI_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `anthropic`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`, `kilocode` (default: `auto`) |
| `VULTI_PORTAL_BASE_URL` | Override Nous Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override Nous inference API URL |
| `VULTI_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `VULTI_NOUS_TIMEOUT_SECONDS` | HTTP timeout for Nous credential / token flows |
| `VULTI_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `VULTI_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `VULTI_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.vulti/sandboxes/`) |
...
| Variable | Description |
|----------|-------------|
| `VULTI_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 60) |
| `VULTI_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `VULTI_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `VULTI_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `VULTI_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `VULTI_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `VULTI_QUIET` | Suppress non-essential output (`true`/`false`) |
| `VULTI_API_TIMEOUT` | LLM API call timeout in seconds (default: `900`) |
| `VULTI_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `VULTI_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
...
For task-specific direct endpoints, Vulti uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in `~/.vulti/config.yaml` under the `provider_routing` section:
...
:::tip
Use `vulti config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
