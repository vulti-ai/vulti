...
description: "Reference for Vulti MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Vulti may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Vulti only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Vulti does not create an empty MCP runtime toolset for that server.
