...
description: "Catalog of bundled skills that ship with Vulti"
---

# Bundled Skills Catalog

Vulti ships with a large built-in skill library copied into `~/.vulti/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `vulti-agent-spawning` | Spawn additional Vulti instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate vulti process. | `autonomous-ai-agents/vulti-agent` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Vulti tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `vulti-atropos-environments` | Build, test, and debug Vulti RL environments for Atropos training. Covers the VultiAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/vulti-atropos-environments` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Vulti venv. | `productivity/google-workspace` |
...
| `plan` | Plan mode for Vulti — inspect context, write a markdown plan into `.vulti/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
