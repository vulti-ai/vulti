...
description: "Frequently asked questions and solutions to common issues with Vulti"
---
...
### What LLM providers work with Vulti?

Vulti works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `vulti model` or by editing `~/.vulti/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Vulti requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Vulti from inside it. The standard install command works perfectly in WSL2:
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Vulti does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.vulti/`.

### Can I use it offline / with local models?

Yes. Point Vulti at any local OpenAI-compatible server:

```bash
vulti config set OPENAI_BASE_URL http://localhost:11434/v1  # Ollama
vulti config set OPENAI_API_KEY ollama                       # Any non-empty value
vulti config set VULTI_MODEL llama3.1
```

You can also save the endpoint interactively with `vulti model`. Vulti persists that custom endpoint in `config.yaml`, and auxiliary tasks configured with provider `main` follow the same saved endpoint.
...
Vulti itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Vulti instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Vulti programmatically:

```python
from vulti.agent import AIAgent

agent = AIAgent(model="openrouter/nous/vulti-3-llama-3.1-70b")
...
#### `vulti: command not found` after installation
...
**Cause:** Vulti requires Python 3.11 or newer.
...
vulti config get OPENROUTER_API_KEY

# Re-configure your provider
vulti model

# Or set directly
vulti config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.vulti/.env` for conflicting entries.
:::
...
# Set a valid model
vulti config set VULTI_MODEL openrouter/nous/vulti-3-llama-3.1-70b

# Or specify per-session
vulti chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `vulti chat --provider <alternative>` to route to a different backend
...
vulti chat --model openrouter/google/gemini-2.0-flash-001
```
...
**Cause:** Vulti detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Vulti never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `vulti chat`
...
**Solution:**
- Verify your bot token is valid with `vulti gateway setup`
- Check gateway logs: `vulti gateway logs`
...
Configure in `~/.vulti/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install vulti-agent[telegram]   # or [discord], [slack], [whatsapp]
...
**Solution:**
- Try a faster/smaller model: `vulti chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `vulti chat -t "terminal"`
...
Verify your `~/.vulti/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
vulti config show | grep -A 12 mcp_servers

# Restart Vulti or reload MCP after config changes
vulti chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Vulti](/docs/guides/use-mcp-with-vulti)
...
:::warning
If an MCP server crashes mid-request, Vulti will report a timeout. Check the server's own logs (not just Vulti logs) to diagnose the root cause.
:::
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/vulti-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Vulti version (`vulti --version`), and the full error message
