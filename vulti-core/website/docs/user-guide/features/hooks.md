...
Each hook is a directory under `~/.vulti/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".vulti" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.vulti/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.vulti/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.vulti/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.vulti/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".vulti" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.vulti/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.vulti/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/vulti-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.vulti/hooks/`
