...
[Honcho](https://honcho.dev) is an AI-native memory system that gives Vulti persistent, cross-session understanding of users. While Vulti has built-in memory (`MEMORY.md` and `USER.md`), Honcho adds a deeper layer of **user modeling** — learning preferences, goals, communication style, and context across conversations via a dual-peer architecture where both the user and the AI build representations over time.

## Works Alongside Built-in Memory

Vulti has two memory systems that can work together or be configured separately. In `hybrid` mode (the default), both run side by side — Honcho adds cross-session user modeling while local files handle agent-level notes.

| Feature | Built-in Memory | Honcho Memory |
|---------|----------------|---------------|
| Storage | Local files (`~/.vulti/memories/`) | Cloud-hosted Honcho API |
...
`apiKey` lives at the root because it is a shared credential across all Honcho-enabled tools. All other settings are scoped under `hosts.vulti`. The `vulti honcho setup` wizard writes this structure automatically.

Or set the API key as an environment variable:

```bash
vulti config set HONCHO_API_KEY your-key
```
...
Settings are scoped to `hosts.vulti` and fall back to root-level globals when the host field is absent. Root-level keys are managed by the user or the honcho CLI -- Vulti only writes to its own host block (except `apiKey`, which is a shared credential at root).
...
**Host-level (`hosts.vulti`)**

| Field | Default | Description |
|-------|---------|-------------|
| `workspace` | `"vulti"` | Workspace identifier |
| `peerName` | *(derived)* | Your identity name for user modeling |
| `aiPeer` | `"vulti"` | AI assistant identity name |
...
All host-level fields fall back to the equivalent root-level key if not set under `hosts.vulti`. Existing configs with settings at root level continue to work.
...
### Vulti Config (`~/.vulti/config.yaml`)
...
```
vulti honcho setup                        # Interactive setup wizard
vulti honcho status                       # Show config and connection status
...
vulti honcho tokens --context N           # Set context token cap
vulti honcho tokens --dialectic N         # Set dialectic char cap
vulti honcho identity                     # Show AI peer identity
vulti honcho identity <file>              # Seed AI peer identity from file (SOUL.md, etc.)
vulti honcho migrate                      # Migration guide: OpenClaw → Vulti + Honcho
```

### Doctor Integration

`vulti doctor` includes a Honcho section that validates config, API key, and connection status.
...
```bash
vulti honcho identity ~/.vulti/SOUL.md
```
