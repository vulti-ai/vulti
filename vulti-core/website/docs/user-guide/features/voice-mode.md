...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Vulti — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Vulti supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Vulti](/docs/guides/use-voice-mode-with-vulti).

## Prerequisites

Before using voice features, make sure you have:

1. **Vulti installed** — `pip install vulti-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `vulti model` or set your preferred provider credentials in `~/.vulti/.env`
3. **A working base setup** — run `vulti` to verify the agent responds to text before enabling voice

:::tip
The `~/.vulti/` directory and default `config.yaml` are created automatically the first time you run `vulti`. You only need to create `~/.vulti/.env` manually for API keys.
:::
...
pip install vulti-agent[tts-premium]

# Everything at once
pip install vulti-agent[all]
```
...
```bash
vulti                # Start the interactive CLI
```
...
1. Start the CLI with `vulti` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.vulti/config.yaml` (default: `ctrl+b`).
:::
...
```bash
vulti gateway        # Start the gateway (connects to configured platforms)
vulti gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@vultibyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.vulti/.env`:
```bash
...
```bash
vulti gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.vulti/.env`
...
- Review gateway logs: `tail -f ~/.vulti/logs/gateway.log`
