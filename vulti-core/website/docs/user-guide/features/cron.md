...
Schedule tasks to run automatically with natural language or cron expressions. Vulti exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Vulti disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
vulti cron create "every 2h" "Check server status"
vulti cron create "every 1h" "Summarize new feed items" --skill blogwatcher
vulti cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Vulti will use the unified `cronjob` tool internally.
...
```bash
vulti cron edit <job_id> --schedule "every 4h"
vulti cron edit <job_id> --prompt "Use the revised task"
vulti cron edit <job_id> --skill blogwatcher --skill find-nearby
vulti cron edit <job_id> --add-skill find-nearby
vulti cron edit <job_id> --remove-skill blogwatcher
vulti cron edit <job_id> --clear-skills
```
...
```bash
vulti cron list
vulti cron pause <job_id>
vulti cron resume <job_id>
vulti cron run <job_id>
vulti cron remove <job_id>
vulti cron status
vulti cron tick
```
...
sudo vulti gateway install --system   # Linux: boot-time system service for servers
...
1. loads jobs from `~/.vulti/cron/jobs.json`
...
A file lock at `~/.vulti/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.vulti/cron/output/`) | Default on CLI |
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Vulti skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.vulti/cron/jobs.json`. Output from job runs is saved to `~/.vulti/cron/output/{job_id}/{timestamp}.md`.
