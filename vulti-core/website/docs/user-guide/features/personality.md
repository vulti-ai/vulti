...
description: "Customize Vulti's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Vulti's personality is customizable, but there are two different layers that matter:

- `SOUL.md` — a durable persona file that lives in `VULTI_HOME` and is loaded automatically for that Vulti instance
...
Vulti now seeds a default `SOUL.md` automatically in:

```text
~/.vulti/SOUL.md
```

More precisely, it uses the current instance's `VULTI_HOME`, so if you run Vulti with a custom home directory, it will use:
...
- Vulti creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Vulti loads `SOUL.md` only from `VULTI_HOME`
- Vulti does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, Vulti adds nothing from it to the prompt
- If `SOUL.md` has content, that content is injected verbatim after security scanning and truncation
- Vulti does not add wrapper language like "If SOUL.md is present..." around the file anymore
...
If Vulti loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `VULTI_HOME`, the personality belongs to the Vulti instance itself.

That also makes it easier to teach users:
- "Edit `~/.vulti/SOUL.md` to change Vulti' default personality."
...
- how Vulti should handle uncertainty, disagreement, or ambiguity
...
## What Vulti injects into the prompt

If `SOUL.md` contains text, Vulti injects the file's text itself — not a wrapper explanation.
...
Vulti ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Vulti, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Vulti its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.vulti/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.vulti/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-vulti)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Vulti speaks
- `display.skin` and `/skin` affect how Vulti looks in the terminal
