...
Vulti includes a full browser automation toolset that can run in two modes:
...
If you do **not** set Browserbase credentials, Vulti can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `vulti config set toolsets '["vulti-cli", "browser"]'`.
:::
...
```
Type "vulti agent" into the search field @e3
```
...
Screenshots are stored in `~/.vulti/browser_screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.vulti/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Vulti automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
