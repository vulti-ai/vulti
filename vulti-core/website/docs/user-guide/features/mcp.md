...
description: "Connect Vulti to external tool servers via MCP — and control exactly which MCP tools Vulti loads"
---

# MCP (Model Context Protocol)

MCP lets Vulti connect to external tool servers so the agent can use tools that live outside Vulti itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Vulti to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Vulti tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Vulti to see
...
2. Add an MCP server to `~/.vulti/config.yaml`:
...
4. Ask Vulti to use the MCP-backed capability.
...
Vulti will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Vulti connects to directly.
...
- you do not want Vulti spawning a local subprocess for that integration

## Basic configuration reference

Vulti reads MCP config from `~/.vulti/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Vulti skips the server entirely |
...
## How Vulti registers MCP tools

Vulti prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Vulti sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Vulti also registers utility tools around MCP resources and prompts:
...
- Vulti only registers resource utilities if the MCP session actually supports resource operations
- Vulti only registers prompt utilities if the MCP session actually supports prompt operations
...
You can now control which tools each MCP server contributes to Vulti.
...
If `enabled: false`, Vulti skips the server completely and does not even attempt a connection.
...
You can also separately disable Vulti-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Vulti does not create an empty runtime MCP toolset for that server.
...
Vulti discovers MCP servers at startup and registers their tools into the normal tool registry.
...
For stdio servers, Vulti does not blindly pass your full shell environment.
...
Then verify your config and restart Vulti.
...
Because Vulti now only registers those wrappers when both are true:
