...
description: "Overview of Vulti's tools — what's available, how toolsets work, and terminal backends"
---
...
Vulti ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, Honcho memory, and more.
...
vulti chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `vulti-cli`, `vulti-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
vulti config set terminal.backend singularity
vulti config set terminal.singularity_image ~/python.sif
```
...
modal setup
vulti config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.vulti/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.vulti/.env`.
:::
