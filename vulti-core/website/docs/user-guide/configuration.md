...
sidebar_position: 2
title: "Configuration"
description: "Configure Vulti — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.vulti/` directory for easy access.
...
```bash
vulti config              # View current configuration
vulti config edit         # Open config.yaml in your editor
vulti config set KEY VAL  # Set a specific value
vulti config check        # Check for missing options (after updates)
vulti config migrate      # Interactively add missing options

# Examples:
vulti config set model anthropic/claude-opus-4
vulti config set terminal.backend docker
vulti config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `vulti config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `vulti chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.vulti/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.vulti/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You need at least one way to connect to an LLM. Use `vulti model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `vulti model` (OAuth, subscription-based) |
| **OpenAI Codex** | `vulti model` (ChatGPT OAuth, uses Codex models) |
| **Anthropic** | `vulti model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.vulti/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.vulti/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.vulti/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.vulti/.env` (provider: `kimi-coding`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.vulti/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.vulti/.env` (provider: `minimax-cn`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.vulti/.env` (provider: `kilocode`) |
| **Custom Endpoint** | `vulti model` (saved in `config.yaml`) or `OPENAI_BASE_URL` + `OPENAI_API_KEY` in `~/.vulti/.env` |

:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Vulti stores the resulting credentials in its own auth store under `~/.vulti/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::
...
vulti chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `vulti model`
# Vulti will use Claude Code's credential store directly when available
vulti model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
vulti chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
vulti chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `vulti model`, Vulti prefers Claude Code's own credential store over copying the token into `~/.vulti/.env`. That keeps refreshable Claude credentials refreshable.
...
```bash
# z.ai / ZhipuAI GLM
vulti chat --provider zai --model glm-4-plus
# Requires: GLM_API_KEY in ~/.vulti/.env

# Kimi / Moonshot AI
vulti chat --provider kimi-coding --model moonshot-v1-auto
# Requires: KIMI_API_KEY in ~/.vulti/.env

# MiniMax (global endpoint)
vulti chat --provider minimax --model MiniMax-Text-01
# Requires: MINIMAX_API_KEY in ~/.vulti/.env

# MiniMax (China endpoint)
vulti chat --provider minimax-cn --model MiniMax-Text-01
# Requires: MINIMAX_CN_API_KEY in ~/.vulti/.env
```
...
Vulti works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Vulti at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`vulti model` and the manual `.env` approach end up in the same runtime path. If you save a custom endpoint through `vulti model`, Vulti persists the provider + base URL in `config.yaml` so later sessions keep using that endpoint even if `OPENAI_BASE_URL` is not exported in your current shell.
...
vLLM supports tool calling, structured output, and multi-modal models. Use `--enable-auto-tool-choice` and `--tool-call-parser vulti` for Vulti-format tool calling with NousResearch models.
...
:::tip
You can switch between providers at any time with `vulti model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
By default, Vulti uses the [Firecrawl cloud API](https://firecrawl.dev/) for web search and scraping. If you prefer to run Firecrawl locally, you can point Vulti at a self-hosted instance instead.
...
2. Point Vulti at your instance (no API key needed):
   ```bash
   vulti config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.vulti/config.yaml`:
...
Configure a backup provider:model that Vulti switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
Optional cheap-vs-strong routing lets Vulti keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, Vulti may route it to `cheap_model`
- If the turn looks complex, Vulti stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, Vulti falls back to the primary model automatically
...
- **Docker backend**
  ...
  - Vulti needs to be able to find the `docker` CLI. It checks your `$PATH` first and also probes common Docker Desktop install locations on macOS. Run:
    ```bash
    docker version
    ```
    If this fails, fix your Docker installation or switch back to the local backend:
    ```bash
    vulti config set terminal.backend local
    ```
...
- **SSH backend**
  - Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set, for example:
    ...
    ```
  - If either value is missing, Vulti will log a clear error and refuse to use the SSH backend.

- **Modal backend**
  - You need either a `MODAL_TOKEN_ID` environment variable or a `~/.modal.toml` config file.
  - If neither is present, the backend check fails and Vulti will report that the Modal backend is not available.
...
Vulti resolves each listed variable from your current shell first, then falls back to `~/.vulti/.env` if it was saved with `vulti config set`.
...
Docker sandboxes stay isolated by default. Vulti does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Vulti from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Vulti from
...
```bash
vulti config set terminal.persistent_shell false
```
...
```yaml
worktree: true    # Always create a worktree (same as vulti -w)
...
Vulti uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via OpenRouter or Nous Portal — you don't need to configure anything.

To use a different model, add an `auxiliary` section to `~/.vulti/config.yaml`:
...
Or via environment variable (in `~/.vulti/.env`):
...
| `"nous"` | Force Nous Portal | `vulti login` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `vulti model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `vulti model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Vulti uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` follows the same custom endpoint Vulti uses for normal chat. That endpoint can be set directly with `OPENAI_BASE_URL`, or saved once through `vulti model` and persisted in `config.yaml`.
...
:::tip
Run `vulti config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
If the requested provider is unavailable, Vulti falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.vulti/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Vulti to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Vulti still keys DMs by chat/DM ID as usual.
...
quick_commands:
  status:
    type: exec
    command: systemctl status vulti-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.vulti/vulti-agent && git pull && pip install -e .
  gpu:
    type: exec
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.vulti/browser_recordings/
```
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: vulti --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Vulti falls back to `OPENAI_API_KEY` only.
...
Vulti uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `AGENTS.md` | Project-specific instructions, coding conventions | Working directory / project tree |
| `SOUL.md` | Default persona for this Vulti instance | `~/.vulti/SOUL.md` or `$VULTI_HOME/SOUL.md` |
...
- **SOUL.md** is now global to the Vulti instance and is loaded only from `VULTI_HOME`.
- Vulti automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`vulti`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.vulti/.env or ~/.vulti/config.yaml:
