...
Vulti automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.vulti/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.vulti/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`vulti` or `vulti chat`) |
...
vulti -c "my project"   # → resumes "my project #3"
```
...
vulti --resume 20250305_091523_a1b2c3d4
vulti -r 20250305_091523_a1b2c3d4

# Resume by title
vulti --resume "refactoring auth"

# Or with the chat subcommand
vulti chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `vulti sessions list`.

### Conversation Recap on Resume

When you resume a session, Vulti displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Vulti session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.vulti/config.yaml`:
...
```bash
vulti sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Vulti creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`vulti -c "my project"`), it automatically picks the most recent session in the lineage.
...
Vulti provides a full set of session management commands via `vulti sessions`:
...
# Filter by platform
vulti sessions list --source telegram

# Show more sessions
vulti sessions list --limit 50
```
...
vulti sessions export backup.jsonl

# Export sessions from a specific platform
vulti sessions export telegram-history.jsonl --source telegram

# Export a single session
vulti sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
vulti sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
vulti sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
vulti sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
vulti sessions prune

# Custom age threshold
vulti sessions prune --older-than 30

# Only prune sessions from a specific platform
vulti sessions prune --source telegram --older-than 60

# Skip confirmation
vulti sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`vulti insights`](/docs/reference/cli-commands#vulti-insights).
...
When Vulti cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Vulti uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Vulti in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.vulti/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.vulti/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.vulti/sessions/sessions.json` | Maps session keys to active session IDs |
...
vulti sessions delete <session_id>
