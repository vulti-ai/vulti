...
Vulti is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Vulti checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
The `VULTI_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.vulti/config.yaml`:
...
:::tip
Use `vulti config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Vulti controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.vulti/.env`:
...
For more flexible authorization, Vulti includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `vulti pairing approve <platform> <code>` on the CLI
...
vulti pairing approve telegram ABC12DEF

# Revoke a user's access
vulti pairing revoke telegram 123456789

# Clear all pending codes
vulti pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.vulti/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Vulti applies strict security hardening to every container.
...
Container resources are configurable in `~/.vulti/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.vulti/sandboxes/docker/<task_id>/`
...
4. **Store secrets securely** — keep API keys in `~/.vulti/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.vulti/logs/` for unauthorized access attempts
10. **Keep updated** — run `vulti update` regularly for security patches
