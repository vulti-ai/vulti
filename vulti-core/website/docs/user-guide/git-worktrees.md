...
sidebar_position: 9
title: "Git Worktrees"
description: "Run multiple Vultis safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Vulti is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Vulti so each session has a clean, isolated working directory.

## Why Use Worktrees with Vulti?

Vulti treats the **current working directory** as the project root:

- CLI: the directory where you run `vulti` or `vulti chat`
...
git worktree add ../repo-feature feature/vulti-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/vulti-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Vulti there:

```bash
cd ../repo-feature

# Start Vulti in the worktree
vulti
```
...
git worktree add ../repo-experiment-a feature/vulti-a
git worktree add ../repo-experiment-b feature/vulti-b
```
...
- Works on its own branch (`feature/vulti-a` vs `feature/vulti-b`).
...
- Vulti checkpoint data under `~/.vulti/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Vulti experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/vulti-checkpoints-docs`, `feature/vulti-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Vulti from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `vulti -w` (Automatic Worktree Mode)

Vulti has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `vulti/vulti-<hash>`).
...
```bash
vulti -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `vulti -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Vulti session its own clean checkout.
