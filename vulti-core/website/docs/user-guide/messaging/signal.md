...
sidebar_position: 6
title: "Signal"
description: "Set up Vulti as a Signal messenger bot via signal-cli daemon"
---

# Signal Setup

Vulti connects to Signal through the [signal-cli](https://github.com/AsamK/signal-cli) daemon running in HTTP mode. The adapter streams messages in real-time via SSE (Server-Sent Events) and sends responses via JSON-RPC.
...
The Signal adapter uses `httpx` (already a core Vulti dependency) for all communication. No additional Python packages are required. You just need signal-cli installed externally.
:::
...
DM access follows the same pattern as all other Vulti platforms:

1. **`SIGNAL_ALLOWED_USERS` set** → only those users can message
2. **No allowlist set** → unknown users get a DM pairing code (approve via `vulti pairing approve signal CODE`)
...
- This applies to both Vulti gateway logs and the global redaction system
