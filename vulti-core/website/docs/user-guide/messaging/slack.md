...
sidebar_position: 4
title: "Slack"
description: "Set up Vulti as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Vulti to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Vulti instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Vulti uses the modern
...
4. Enter an app name (e.g., "Vulti") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `vulti-socket` (the name doesn't matter)
...
Vulti uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
vulti gateway setup    # Select Slack when prompted
```
...
Understanding how Vulti behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Vulti what time is it?`). In channels, Vulti replies in a thread attached to that message. |
| **Threads** | If you @mention Vulti inside an existing thread, it replies in that same thread. |
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Vulti will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Vulti`).

---

## Voice Messages

Vulti supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Vulti` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Vulti` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Vulti`)
...
- Tokens should be stored in `~/.vulti/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Vulti config directory
