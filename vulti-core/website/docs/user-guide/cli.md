...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Vulti terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Vulti's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
vulti chat --model "anthropic/claude-sonnet-4"

# With a specific provider
vulti chat --provider nous        # Use Nous Portal
vulti chat --provider openrouter  # Force OpenRouter
...
vulti chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
vulti --continue             # Resume the most recent CLI session (-c)
vulti --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
vulti chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
vulti -w                         # Interactive mode in worktree
vulti -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Vulti CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Vulti CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`vulti -c` or `vulti --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
Type `/` to see the autocomplete dropdown. Vulti supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Vulti replies |
...
```bash
vulti -s vulti-agent-dev,github-auth
vulti chat -s github-pr-workflow -s github-auth
```

Vulti loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.vulti/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.vulti/config.yaml`:
...
agent:
  personalities:
    ...
    pirate: "Arrr! Ye be talkin' to Captain Vulti..."
    # Add your own!
```
...
Resume this session with:
  vulti --resume 20260225_143052_a1b2c3
...
```bash
vulti --continue                          # Resume the most recent CLI session
vulti -c                                  # Short form
vulti -c "my project"                     # Resume a named session (latest in lineage)
vulti --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
vulti --resume "refactoring auth"         # Resume by title
vulti -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `vulti sessions rename <id> <title>` from the command line. Use `vulti sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Vulti's SQLite state database under `~/.vulti/state.db`. The database keeps:
...
Vulti immediately confirms the task and gives you back the prompt:
