---
sidebar_position: 5
title: "Use MCP with Vulti"
description: "A practical guide to connecting MCP servers to Vulti, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Vulti

This guide shows how to actually use MCP with Vulti in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Vulti tool
- you want Vulti to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Vulti to internal APIs, databases, or company systems without modifying Vulti core

Do not use MCP when:
- a built-in Vulti tool already solves the job well
...
- Vulti remains the agent
- MCP servers contribute tools
- Vulti discovers those tools at startup or reload time
...
- Vulti banner/status should show MCP integration when configured
- ask Vulti what tools it has available
...
There are two categories of MCP-exposed functionality in Vulti:
...
2. Vulti-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Vulti will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Vulti to reason over a bounded workspace.
...
Now Vulti can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Vulti core.
...
Because Vulti now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
