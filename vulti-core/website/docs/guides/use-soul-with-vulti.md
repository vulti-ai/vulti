---
sidebar_position: 6
title: "Use SOUL.md with Vulti"
description: "How to use SOUL.md to shape Vulti's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Vulti

`SOUL.md` is the easiest way to give Vulti a stable, default voice.

If you want Vulti to feel like the same assistant every time you talk to it — without repeating instructions in every session — this is the file to use.
...
- personality
- communication style
- how direct or warm Vulti should be
- what Vulti should avoid stylistically
- how Vulti should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Vulti is and how Vulti speaks
...
Vulti now uses only the global SOUL file for the current instance:

```text
~/.vulti/SOUL.md
```

If you run Vulti with a custom home directory, it becomes:
...
Vulti automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Vulti does not overwrite it
- if the file exists but is empty, Vulti adds nothing from it to the prompt

## How Vulti uses it

When Vulti starts a session, it reads `SOUL.md` from `VULTI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and injects the content directly into the prompt.

No wrapper language is added around the file.

So the content itself matters. Write the way you want Vulti to think and speak.
...
That alone can noticeably change how Vulti feels.
...
Vulti already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Vulti should sound.

# Avoid
What Vulti should not do.

# Defaults
How Vulti should behave when ambiguity appears.
```
...
Then restart Vulti or start a new session.
...
### I edited SOUL.md but Vulti still sounds the same

Check:
- you edited `~/.vulti/SOUL.md` or `$VULTI_HOME/SOUL.md`
...
### Vulti is ignoring parts of my SOUL.md
