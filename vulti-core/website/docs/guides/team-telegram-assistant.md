...
This tutorial walks you through setting up a Telegram bot powered by Vulti that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Vulti installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.vulti/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Vulti itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Vulti Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_vulti_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Vulti. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Vulti Gateway...
...
sudo vulti gateway install --system   # Linux only: boot-time system service
```
...
# View live logs
journalctl --user -u vulti-gateway -f
...
```bash
vulti gateway stop && vulti gateway start
```
...
vulti pairing revoke telegram 987654321

# Clear expired pending codes
vulti pairing clear-pending
```
...
**Option 2:** Set it manually in `~/.vulti/.env`:
...
Control how much detail the bot shows when using tools. In `~/.vulti/config.yaml`:
...
Customize how the bot communicates by editing `~/.vulti/SOUL.md`:

For a full guide, see [Use SOUL.md with Vulti](/docs/guides/use-soul-with-vulti).
...
```bash
# From the CLI
vulti cron list          # View all scheduled jobs
vulti cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u vulti-gateway` (Linux) or `~/.vulti/logs/gateway.log` (macOS) |
| Cron job output | `~/.vulti/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.vulti/cron/jobs.json` |
| Pairing data | `~/.vulti/pairing/` |
| Session history | `~/.vulti/sessions/` |
