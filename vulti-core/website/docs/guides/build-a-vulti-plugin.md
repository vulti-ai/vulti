...
# Build a Vulti Plugin

This guide walks through building a complete Vulti plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.vulti/plugins/calculator
cd ~/.vulti/plugins/calculator
```
...
This tells Vulti: "I'm a plugin called calculator, I provide tools and hooks." That's all the manifest needs.
...
4. **Accept `**kwargs`:** Vulti may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but Vulti continues fine
...
def _install_skill():
    """Copy our skill to ~/.vulti/skills/ on first load."""
    try:
        from vulti_cli.config import get_vulti_home
        dest = get_vulti_home() / "skills" / "my-plugin" / "SKILL.md"
    except Exception:
        dest = Path.home() / ".vulti" / "skills" / "my-plugin" / "SKILL.md"
...
```toml
# pyproject.toml
[project.entry-points."vulti_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install vulti-plugin-calculator
# Plugin auto-discovered on next vulti startup
```
...
```python
# Wrong — will break if Vulti passes extra context
def handler(args):
    ...
