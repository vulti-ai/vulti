...
2. **Vulti spins up** a fresh agent session with your prompt
...
- **Vulti installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.vulti/cron/output/` and you can read them anytime.
:::
...
Vulti will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Vulti
```
...
Vulti will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Vulti to delegate each topic to a sub-agent:
...
Vulti will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
