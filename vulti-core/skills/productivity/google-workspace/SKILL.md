...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Vulti venv.
version: 1.0.0
...
```bash
GSETUP="python ~/.vulti/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/.vulti/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.vulti/google_oauth_pending.json` until exchange completes.
...
```bash
GAPI="python ~/.vulti/skills/productivity/google-workspace/scripts/google_api.py"
```
