---
name: vulti-agent-spawning
description: Spawn additional Vulti instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate vulti process.
version: 1.1.0
author: Vulti
license: MIT
metadata:
  vulti:
    tags: [Agent, Vulti, Multi-Agent, Orchestration, Subprocess, Interactive]
    homepage: https://github.com/NousResearch/vulti-agent
    related_skills: [claude-code, codex]
---

# Spawning Vulti Instances

Run additional Vulti processes as autonomous subprocesses. Unlike `delegate_task` (which spawns lightweight subagents sharing the same process), this launches fully independent `vulti` CLI processes with their own sessions, tools, and terminal environments.

## When to Use This vs delegate_task

| Feature | `delegate_task` | Spawning `vulti` process |
|---------|-----------------|--------------------------|
...
- `vulti` CLI installed and on PATH
- API key configured in `~/.vulti/.env`
...
This installs uv, Python 3.11, clones the repo, sets up the venv, and launches an interactive setup wizard to configure your API provider and model. See the [GitHub repo](https://github.com/NousResearch/vulti-agent) for details.
...
terminal(command="vulti --continue", background=true, pty=true)

# Resume a specific session by ID (shown on exit)
terminal(command="vulti --resume 20260225_143052_a1b2c3", background=true, pty=true)
```
...
```
terminal(command="vulti chat -q 'Research the latest GRPO training papers and write a summary to ~/research/grpo.md'", timeout=300)
```

Background for long tasks:
```
terminal(command="vulti chat -q 'Set up CI/CD for ~/myapp'", background=true)
# Returns session_id, monitor with process tool
```

## Mode 2: Interactive PTY Session

Launch a full interactive Vulti session with PTY for back-and-forth collaboration. You can send messages, review its work, give feedback, and steer it.

Note: Vulti uses prompt_toolkit for its CLI UI. Through a PTY, this works because ptyprocess provides a real terminal — input sent via `submit` arrives as keystrokes. The output log will contain ANSI escape sequences from the UI rendering — focus on the text content, not the formatting.

```
# Start interactive vulti in background with PTY
terminal(command="vulti", workdir="~/project", background=true, pty=true)
# Returns session_id
...
**Code review loop** — spawn vulti, send code for review, iterate on feedback:
```
terminal(command="vulti", workdir="~/project", background=true, pty=true)
...
```
terminal(command="vulti", background=true, pty=true)
...
```
# Agent A: backend
terminal(command="vulti", workdir="~/project/backend", background=true, pty=true)
process(action="submit", session_id="<agent-a>", data="Build a REST API for user management with CRUD endpoints")

# Agent B: frontend
terminal(command="vulti", workdir="~/project/frontend", background=true, pty=true)
...
```
terminal(command="vulti chat -q 'Research competitor landing pages and write a report to ~/research/competitors.md'", background=true)
terminal(command="vulti chat -q 'Audit security of ~/myapp and write findings to ~/myapp/SECURITY_AUDIT.md'", background=true)
process(action="list")
```
...
```
terminal(command="vulti chat -q 'Summarize this codebase' --model google/gemini-2.5-pro", workdir="~/project", background=true)
```
...
```
# Start vulti inside tmux
tmux new-session -d -s vulti-session -x 120 -y 40 "vulti"
sleep 10  # Wait for banner/startup

# Send messages
tmux send-keys -t vulti-session "your message here" Enter

# Read output
sleep 15  # Wait for LLM response
tmux capture-pane -t vulti-session -p

# Multi-turn: just send more messages and capture again
tmux send-keys -t vulti-session "follow-up message" Enter

# Exit when done
tmux send-keys -t vulti-session "/exit" Enter
tmux kill-session -t vulti-session
```
