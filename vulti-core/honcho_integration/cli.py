...
Handles: vulti honcho setup | status | sessions | map | peer
"""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Vulti persistent cross-session memory.")
    print("  Config is shared with other hosts at ~/.honcho/config.json\n")

    if not _ensure_sdk_installed():
        return

    # All writes go to hosts.vulti — root keys are managed by the user
    # or the honcho CLI only.
    hosts = cfg.setdefault("hosts", {})
    vulti_host = hosts.setdefault(HOST, {})
    ...
    if not effective_key:
        ...
        print("  Run 'vulti honcho setup' again once you have a key.\n")
        return

    # Peer name
    current_peer = vulti_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        vulti_host["peerName"] = new_peer

    current_workspace = vulti_host.get("workspace") or cfg.get("workspace", "vulti")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        vulti_host["workspace"] = new_workspace

    vulti_host.setdefault("aiPeer", HOST)

    # Memory mode
    current_mode = vulti_host.get("memoryMode") or cfg.get("memoryMode", "hybrid")
    print(f"\n  Memory mode options:")
    ...
    if new_mode in ("hybrid", "honcho"):
        vulti_host["memoryMode"] = new_mode
    else:
        vulti_host["memoryMode"] = "hybrid"

    # Write frequency
    current_wf = str(vulti_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    ...
    try:
        vulti_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        vulti_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # Recall mode
    _raw_recall = vulti_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        vulti_host["recallMode"] = new_recall

    # Session strategy
    current_strat = vulti_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print(f"\n  Session strategy options:")
    print("    per-session   — new Honcho session each run, named by Vulti session ID (default)")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        vulti_host["sessionStrategy"] = new_strat

    vulti_host.setdefault("enabled", True)
    vulti_host.setdefault("saveMessages", True)
    ...
    print(f"\n  Other commands:")
    print(f"    vulti honcho status     — show full config")
    print(f"    vulti honcho mode       — show or change memory mode")
    print(f"    vulti honcho tokens     — show or set token budgets")
    print(f"    vulti honcho identity   — seed or show AI peer identity")
    print(f"    vulti honcho map <name> — map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: vulti honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'vulti honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: vulti honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        vulti = hosts.get(HOST, {})
        user = vulti.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = vulti.get('aiPeer') or cfg.get('aiPeer') or HOST
        lvl = vulti.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = vulti.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print(f"\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print(f"    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print(f"    Vulti' identity in Honcho. Seed with 'vulti honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: vulti honcho mode [hybrid|honcho]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = vulti.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = vulti.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = vulti.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print(f"\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print(f"    AI-to-AI inference. Vulti asks Honcho's AI peer a question")
        ...
        print(f"\n  Set with: vulti honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'vulti honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'vulti honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    vulti honcho identity --show        — show both peer representations")
        print("    vulti honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Vulti + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Vulti\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Vulti persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  vulti honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'vulti honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'vulti honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  vulti honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'vulti'")
        print("  with Honcho configured and no prior session history.")
        print("  (Vulti calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print(f"    vulti honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'vulti honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Vulti, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'vulti honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    vulti honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  vulti honcho identity <path/to/SOUL.md>")
    ...
    print("    Vulti:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Vulti:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Vulti:   Honcho observes every message and updates representations")
    ...
    print("    Vulti:   per-session by default — each run gets its own session")
    print("              Map a custom name:  vulti honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. vulti honcho setup              — configure API key (required)")
        print("  2. vulti honcho migrate            — re-run this walkthrough")
    else:
        print("  1. vulti honcho status             — verify Honcho connection")
        print("  2. vulti                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. vulti honcho identity --show    — verify AI peer representation")
        print("  4. vulti honcho tokens             — tune context and dialectic budgets")
        print("  5. vulti honcho mode               — view or change memory mode")
    print()
