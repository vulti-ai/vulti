...
def _resolve_memory_mode(
    ...
    Object form:  { "default": "hybrid", "vulti": "honcho", ... }
...
class HonchoClientConfig:
    ...
    # Config object form: "memoryMode": { "default": "hybrid", "vulti": "honcho" }
    ...
    # Max chars of dialectic result to inject into Vulti system prompt
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "vulti") -> HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Vulti session title (from /title command)
          3. per-session strategy — Vulti session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Vulti session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key:
        raise ValueError(
            ...
            "then run 'vulti honcho setup' or set HONCHO_API_KEY."
        )
