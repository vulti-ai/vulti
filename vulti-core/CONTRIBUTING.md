# Contributing to Vulti

Thank you for contributing to Vulti! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Vulti to work everywhere.
...
Bundled skills (in `skills/`) ship with every Vulti install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `vulti skills browse` (labeled "official") and install it with `vulti skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `vulti skills install`.
...
```bash
mkdir -p ~/.vulti/{cron,sessions,logs,memories,skills}
...
├── cli.py                    # VultiCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (vulti-cli, vulti-telegram, etc.)
├── vulti_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── vulti_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.vulti/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (vulti-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.vulti/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.vulti/`)

| Path | Purpose |
|------|---------|
| `~/.vulti/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.vulti/.env` | API keys and secrets |
| `~/.vulti/auth.json` | OAuth credentials (Nous Portal) |
| `~/.vulti/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.vulti/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.vulti/state.db` | SQLite session database |
| `~/.vulti/sessions/` | JSON session logs |
| `~/.vulti/cron/` | Scheduled job data |
| `~/.vulti/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`vulti_state.py`) with full-text search and unique session titles. JSON logs go to `~/.vulti/sessions/`.
...
Four fields are supported under `metadata.vulti`:
...
The user may skip setup and keep loading the skill. Vulti only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `vulti setup` or update `~/.vulti/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Vulti tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `vulti --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Vulti uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.vulti/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `vulti_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `vulti_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Vulti runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Vulti has terminal access. Security matters.
...
2. **Test manually**: Run `vulti` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/vulti-agent/issues)
- Include: OS, Python version, Vulti version (`vulti version`), full error traceback
