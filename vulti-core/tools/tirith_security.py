...
it is automatically downloaded from GitHub releases to $VULTI_HOME/bin/tirith.
...
def _get_vulti_home() -> str:
    """Return the Vulti home directory, respecting VULTI_HOME env var.

    Matches the convention used throughout the codebase (vulti_cli.config,
    cli.py, gateway/run.py, etc.) so tirith state stays inside the active
    profile and tests get automatic isolation via conftest's VULTI_HOME
    monkeypatch.
    """
    return os.getenv("VULTI_HOME") or os.path.join(os.path.expanduser("~"), ".vulti")


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_vulti_home(), ".tirith-install-failed")
...
def _vulti_bin_dir() -> str:
    """Return $VULTI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_vulti_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $VULTI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_vulti_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $VULTI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $VULTI_HOME/bin/tirith
    ...
    vulti_bin = os.path.join(_vulti_bin_dir(), "tirith")
    if os.path.isfile(vulti_bin) and os.access(vulti_bin, os.X_OK):
        _resolved_path = vulti_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return vulti_bin
