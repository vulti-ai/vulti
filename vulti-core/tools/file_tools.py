...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        # Security: block direct reads of internal Vulti cache/index files
        # to prevent prompt injection via catalog or hub metadata files.
        import pathlib as _pathlib
        _resolved = _pathlib.Path(path).expanduser().resolve()
        _vulti_home = _pathlib.Path("~/.vulti").expanduser().resolve()
        _blocked_dirs = [
            _vulti_home / "skills" / ".hub" / "index-cache",
            _vulti_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal Vulti cache file "
