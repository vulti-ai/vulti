...
LOCAL_STT_COMMAND_ENV = "VULTI_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "VULTI_LOCAL_STT_LANGUAGE"
...
def get_stt_model_from_config() -> Optional[str]:
    """Read the STT model name from ~/.vulti/config.yaml.
    ...
    try:
        import yaml
        cfg_path = Path(os.getenv("VULTI_HOME", Path.home() / ".vulti")) / "config.yaml"
        if cfg_path.exists():
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="vulti-local-stt-") as output_dir:
