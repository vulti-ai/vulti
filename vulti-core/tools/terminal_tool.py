...
def _check_disk_usage_warning():
    ...
    try:
        # Get total size of vulti directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "vulti-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("VULTI_GATEWAY_SESSION")
    ...
    for failure in sudo_failures:
        if failure in output:
            return output + "\n\n💡 Tip: To enable sudo over messaging, add SUDO_PASSWORD to ~/.vulti/.env on the agent machine."
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (VULTI_INTERACTIVE=1).
    ...
    try:
        os.environ["VULTI_SPINNER_PAUSE"] = "1"
        ...
        if "VULTI_SPINNER_PAUSE" in os.environ:
            del os.environ["VULTI_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (VULTI_INTERACTIVE=1):
    ...
    if not sudo_password:
        ...
        if os.getenv("VULTI_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.vulti/.env or environment variables."
        )
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            session_key = os.getenv("VULTI_SESSION_KEY", "")
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    process_registry.pending_watchers.append({
                        ...
                        "thread_id": os.getenv("VULTI_SESSION_THREAD_ID", ""),
                    })
...
def check_terminal_requirements() -> bool:
    ...
    Important: local and singularity backends now use Vulti' own environment
    ...
    try:
        if env_type == "local":
            # Local execution uses Vulti' own LocalEnvironment wrapper and does
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_SANDBOX_DIR: {os.getenv('TERMINAL_SANDBOX_DIR', '~/.vulti/sandboxes')}")
