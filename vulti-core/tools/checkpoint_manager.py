...
Architecture:
    ~/.vulti/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        VULTI_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = Path(os.getenv("VULTI_HOME", Path.home() / ".vulti")) / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("VULTI_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "vulti@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Vulti Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "VULTI_WORKDIR").write_text(
