...
discovers their tools, and registers them into the vulti-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.vulti/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            vulti_home = os.path.expanduser(
                os.getenv(
                    "VULTI_HOME", os.path.join(os.path.expanduser("~"), ".vulti")
                )
            )
            candidates = [
                os.path.join(vulti_home, "node", "bin", resolved_command),
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Vulti registry schema format.
...
def discover_mcp_tools(agent_id: str = None) -> List[str]:
    ...
    if all_tools:
        # Dynamically inject into all vulti-* platform toolsets
        from toolsets import TOOLSETS
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("vulti-"):
