...
Trust levels:
  - builtin:   Ships with Vulti. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.vulti/\.env|\~/\.vulti/\.env',
     "vulti_env_access", "critical", "exfiltration",
     "directly references Vulti secrets file"),
    ...
    (r'\.vulti/config\.yaml|\.vulti/SOUL\.md',
     "vulti_config_mod", "critical", "persistence",
     "references Vulti configuration files directly"),
...
def _get_configured_model() -> str:
    """Load the user's configured model from ~/.vulti/config.yaml."""
    try:
