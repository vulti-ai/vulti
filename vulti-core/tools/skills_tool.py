...
import yaml
from vulti_cli.config import load_env, _ENV_VAR_NAME_RE
from tools.registry import registry

logger = logging.getLogger(__name__)


# All skills live in ~/.vulti/skills/ (seeded from bundled skills/ on install).
...
def _is_gateway_surface() -> bool:
    if os.getenv("VULTI_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("VULTI_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.vulti/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.vulti/skills/mlops/axolotl/SKILL.md -> "mlops"
    """
    try:
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    Resolves platform from ``VULTI_PLATFORM`` env var, falls back to
    ...
    try:
        ...
        resolved_platform = os.getenv("VULTI_PLATFORM")
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("VULTI_PLATFORM")
        if resolved_platform:
            platform_disabled = skills_cfg.get("platform_disabled", {}).get(resolved_platform)
            if platform_disabled is not None:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.vulti/skills/.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``vulti skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.vulti/skills/",
                },
                ensure_ascii=False,
            )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.vulti/skills/): {skill_md}")
        ...
        # Check metadata.vulti.* first (agentskills.io convention), fall back to top-level
        vulti_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            vulti_meta = metadata.get("vulti", {}) or {}

        tags = _parse_tags(vulti_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            vulti_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
