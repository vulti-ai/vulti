...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    try:
        vulti_home = Path(os.environ.get("VULTI_HOME", Path.home() / ".vulti"))
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-vulti_…`` the resulting socket path exceeds the
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes Vulti-managed Node first, then standard system dirs.
        vulti_home = Path(os.environ.get("VULTI_HOME", Path.home() / ".vulti"))
        vulti_node_bin = str(vulti_home / "node" / "bin")

        existing_path = browser_env.get("PATH", "")
        path_parts = [p for p in existing_path.split(":") if p]
        candidate_dirs = [vulti_node_bin] + [p for p in _SANE_PATH.split(":") if p]
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = vulti_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    vulti_home = Path(os.environ.get("VULTI_HOME", Path.home() / ".vulti"))
    screenshots_dir = vulti_home / "browser_screenshots"
