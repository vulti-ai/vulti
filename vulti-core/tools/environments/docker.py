...
def _load_vulti_env_vars() -> dict[str, str]:
    """Load ~/.vulti/.env values without failing Docker command execution."""
    try:
        from vulti_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.vulti/sandboxes/). Non-persistent
    ...
    def execute(self, command: str, cwd: str = "", *,
        ...
        vulti_env = _load_vulti_env_vars() if self._forward_env else {}
        for key in self._forward_env:
            value = os.getenv(key)
            if value is None:
                value = vulti_env.get(key)
