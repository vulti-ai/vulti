...
_OUTPUT_FENCE = "__VULTI_FENCE_a9f7b3__"

# Vulti-internal env vars that should NOT leak into terminal subprocesses.
# These are loaded from ~/.vulti/.env for Vulti' own LLM/provider calls
# but can break external CLIs (e.g. codex) that also honor them.
# See: https://github.com/NousResearch/vulti-agent/issues/1002
#
...
_VULTI_PROVIDER_ENV_FORCE_PREFIX = "_VULTI_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    optional config registry, so new Vulti-managed secrets are blocked
    ...
    try:
        from vulti_cli.config import OPTIONAL_ENV_VARS
    ...
    # Vars not covered above but still Vulti-internal / conflict-prone.
    blocked.update({
        "OPENAI_BASE_URL",
        "OPENAI_API_KEY",
...
_VULTI_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Vulti-managed secrets from a subprocess environment.

    `_VULTI_FORCE_<VAR>` entries in ``extra_env`` opt a blocked variable back in
    intentionally for callers that truly need it.
    """
    sanitized: dict[str, str] = {}

    for key, value in (base_env or {}).items():
        if key.startswith(_VULTI_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _VULTI_PROVIDER_ENV_BLOCKLIST:
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_VULTI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_VULTI_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _VULTI_PROVIDER_ENV_BLOCKLIST:
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("VULTI_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Vulti requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set VULTI_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_VULTI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_VULTI_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _VULTI_PROVIDER_ENV_BLOCKLIST:
            run_env[k] = v
...
class LocalEnvironment(PersistentShellMixin, BaseEnvironment):
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/vulti-local-{self._session_id}"
