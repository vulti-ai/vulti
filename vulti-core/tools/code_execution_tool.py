...
Lets the LLM write a Python script that calls Vulti tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture:
  1. Parent generates a `vulti_tools.py` stub module with RPC functions
...
def generate_vulti_tools_module(enabled_tools: List[str]) -> str:
    """
    Build the source code for the vulti_tools.py stub module.
    ...
    header = '''\
"""Auto-generated Vulti tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["VULTI_RPC_SOCKET"])
...
def execute_code(
    ...
    # --- Set up temp directory with vulti_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="vulti_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"vulti_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated vulti_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_vulti_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "vulti_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["VULTI_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the vulti-agent root is importable in the sandbox so
        # modules like minisweagent_path are available to child scripts.
        _vulti_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _vulti_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("VULTI_TIMEZONE", "").strip()
        if _tz_name:
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``vulti tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Vulti tools programmatically. "
        ...
        f"Available via `from vulti_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-minute timeout, 50KB stdout cap, max 50 tool calls per script. "
        "terminal() is foreground-only (no background or pty).\n\n"
        "Print your final result to stdout. Use Python stdlib (json, re, math, csv, "
        "datetime, collections, etc.) for processing between tool calls.\n\n"
        "Also available (no import needed — built into vulti_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from vulti_tools import {import_str}` "
