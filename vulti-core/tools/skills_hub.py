#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Vulti Skills Hub.
...
Used by vulti_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            vulti_meta = metadata.get("vulti", {})
            if isinstance(vulti_meta, dict):
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                vulti_meta = meta_block.get("vulti", {})
                if isinstance(vulti_meta, dict):
                    tags = vulti_meta.get("tags", [])
