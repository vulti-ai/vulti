...
Configuration is loaded from ~/.vulti/config.yaml under the 'tts:' key.
...
DEFAULT_OUTPUT_DIR = str(Path(os.getenv("VULTI_HOME", Path.home() / ".vulti")) / "audio_cache")
MAX_TEXT_LENGTH = 4000


# ===========================================================================
# Config loader -- reads tts: section from ~/.vulti/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.vulti/config.yaml.
    ...
    try:
        ...
        logger.debug("vulti_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.vulti/config.yaml (tts: section).
    ...
    platform = os.getenv("VULTI_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.vulti/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
