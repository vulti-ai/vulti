...
DANGEROUS_PATTERNS = [
    ...
    (r'\btee\b.*(/etc/|/dev/sd|\.ssh/|\.vulti/\.env)', "overwrite system file via tee"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    session_key = os.getenv("VULTI_SESSION_KEY", "default")
    if is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}

    is_cli = os.getenv("VULTI_INTERACTIVE")
    is_gateway = os.getenv("VULTI_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("VULTI_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("VULTI_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("VULTI_INTERACTIVE")
    is_gateway = os.getenv("VULTI_GATEWAY_SESSION")
    is_ask = os.getenv("VULTI_EXEC_ASK")
