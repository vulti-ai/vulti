...
_loaded_env_paths = load_vulti_dotenv(vulti_home=_vulti_home, project_env=_project_env)
...
from vulti_constants import OPENROUTER_BASE_URL, OPENROUTER_MODELS_URL
...
class _SafeWriter:
    ...
    When vulti-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        # Persistent error log -- always writes WARNING+ to ~/.vulti/logs/errors.log
        ...
        error_log_dir = _vulti_home / "logs"
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://vulti-agent.nousresearch.com",
                        "X-OpenRouter-Title": "Vulti",
        ...
        # Session logs go into ~/.vulti/sessions/ alongside gateway sessions
        vulti_home = Path(os.getenv("VULTI_HOME", Path.home() / ".vulti"))
        self.logs_dir = vulti_home / "sessions"
        ...
        if not skip_memory:
            try:
                from vulti_cli.config import load_config as _load_mem_config
        ...
        if not skip_memory:
            try:
                ...
                print("  Run 'vulti honcho setup' to reconfigure.")
                self._honcho = None
        ...
        try:
            from vulti_cli.config import load_config as _load_skills_config
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if os.getenv("VULTI_DUMP_REQUEST_STDOUT", "").strip().lower() in {"1", "true", "yes", "on"}:
        ...
        *,
        ...
        if not honcho_sess.messages:
            try:
                ...
                mem_dir = str(get_vulti_home() / "memories")
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if "update_own_profile" in self.valid_tool_names or "manage_own_connections" in self.valid_tool_names:
            tool_guidance.append(PROFILE_GUIDANCE)
        if tool_guidance:
            prompt_parts.append(" ".join(tool_guidance))

        # Honcho CLI awareness: tell Vulti about its own management commands
        ...
        if self._honcho and self._honcho_session_key:
            ...
            honcho_block += (
                ...
                "  vulti honcho status                    — show full config + connection\n"
                "  vulti honcho mode [hybrid|honcho]       — show or set memory mode\n"
                "  vulti honcho tokens [--context N] [--dialectic N] — show or set token budgets\n"
                "  vulti honcho peer [--user NAME] [--ai NAME] [--reasoning LEVEL]\n"
                "  vulti honcho sessions                  — list directory→session mappings\n"
                "  vulti honcho map <name>                — map cwd to a session name\n"
                "  vulti honcho identity [<file>] [--show] — seed or show AI peer identity\n"
                "  vulti honcho migrate                   — migration guide from openclaw-honcho\n"
                "  vulti honcho setup                     — full interactive wizard"
            )
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from vulti_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("VULTI_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("VULTI_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("VULTI_API_TIMEOUT", 900.0)),
        }
        ...
        if _is_nous:
            extra_body["tags"] = ["product=vulti-agent"]
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=vulti-agent"]
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if os.getenv("VULTI_DUMP_REQUESTS", "").strip().lower() in {"1", "true", "yes", "on"}:
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in ~/.vulti/.env for Vulti-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in ~/.vulti/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Clear stale keys: vulti config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Legacy cleanup: vulti config set ANTHROPIC_API_KEY \"\"")
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        # If previous turns executed tool calls (api_call_count > 1),
                        # the model did its work via tools and has nothing textual to
                        # add.  Accept the empty response instead of wasting retries.
                        # Common case: memory flush where the model is told "do NOT
                        # respond, just save memories then stop".
                        if api_call_count > 1:
                            logger.debug("Empty content after tool-call turns — accepting as final (api_call_count=%d)", api_call_count)
                            break
