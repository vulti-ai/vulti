...
Usage:
    vulti pairing list              # Show all pending + approved users
    vulti pairing approve <platform> <code>  # Approve a pairing code
    vulti pairing revoke <platform> <user_id> # Revoke user access
    vulti pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle vulti pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: vulti pairing {list|approve|revoke|clear-pending}")
        print("Run 'vulti pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  Run 'vulti pairing list' to see pending codes.\n")
