"""vulti claw — OpenClaw migration commands.

Usage:
    vulti claw migrate              # Interactive migration from ~/.openclaw
    vulti claw migrate --dry-run    # Preview what would be migrated
    vulti claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
"""
...
from vulti_cli.config import get_vulti_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_vulti.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_vulti", script_path)
...
def claw_command(args):
    """Route vulti claw subcommands."""
    action = getattr(args, "claw_action", None)

    if action == "migrate":
        _cmd_migrate(args)
    else:
        print("Usage: vulti claw migrate [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Vulti")
        print()
        print("Run 'vulti claw migrate --help' for migration options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Vulti migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: vulti claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {vulti_home}")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  vulti claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  vulti claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  vulti config set OPENROUTER_API_KEY sk-or-v1-...")
