"""
Configuration management for Vulti.

Config files are stored in ~/.vulti/ for easy access:
- ~/.vulti/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.vulti/.env         - API keys and secrets

This module provides:
- vulti config          - Show current configuration
- vulti config edit     - Open config in editor
- vulti config set      - Set a specific value
- vulti config wizard   - Re-run setup wizard
"""
...
def ensure_vulti_home():
    """Ensure ~/.vulti directory structure exists with secure permissions."""
    home = get_vulti_home()
...
DEFAULT_CONFIG = {
    ...
    # This section is only needed for vulti-specific overrides; everything else
...
OPTIONAL_ENV_VARS = {
    ...
    # VULTI_TOOL_PROGRESS and VULTI_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("VULTI_TOOL_PROGRESS")
            old_mode = get_env_value("VULTI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from VULTI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from VULTI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("VULTI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from VULTI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: vulti config set <key> <value>")
...
#   openai-codex (OAuth — vulti login) — OpenAI Codex
#   nous         (OAuth — vulti login) — Nous Portal
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.vulti/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Vulti env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.vulti/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.vulti/.env."""
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.vulti/.env or environment."""
...
def show_config():
    ...
    print(color("│              🛡 Vulti Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  vulti config edit     # Edit config file", Colors.DIM))
    print(color("  vulti config set <key> <value>", Colors.DIM))
    print(color("  vulti setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: vulti config set <key> <value>")
            print()
            print("Examples:")
            print("  vulti config set model anthropic/claude-sonnet-4")
            print("  vulti config set terminal.backend docker")
            print("  vulti config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'vulti config migrate' to add them")
        ...
        print("  vulti config           Show current configuration")
        print("  vulti config edit      Open config in editor")
        print("  vulti config set <key> <value>   Set a config value")
        print("  vulti config check     Check for missing/outdated config")
        print("  vulti config migrate   Update config with new options")
        print("  vulti config path      Show config file path")
        print("  vulti config env-path  Show .env file path")
        sys.exit(1)
