"""
Unified tool configuration for Vulti.

`vulti tools` and `vulti setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.vulti/config.yaml under
...
# They're still in _VULTI_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl"}

# Platform display config
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "vulti-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "vulti-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "vulti-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "vulti-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "vulti-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "vulti-signal"},
    "email":    {"label": "📧 Email",      "default_toolset": "vulti-email"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning("    npm install failed - run manually: cd ~/.vulti/vulti-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in vulti-agent directory)")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `vulti tools` and `vulti setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("🛡 Vulti Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    print(color("  Tool configuration saved to ~/.vulti/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'vulti' or gateway restart.", Colors.DIM))
    print()
