...
Add, remove, or reorder entries here — both `vulti setup` and
`vulti` provider-selection will pick up the change automatically.
"""
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Vulti' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``vulti_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from vulti_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from vulti_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Vulti will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
