...
into prompt_toolkit's event loop. Each function takes the VultiCLI instance
...
from vulti_cli.banner import cprint, _DIM, _RST
from vulti_cli.config import save_env_value_secure
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.vulti/.env and never exposed to the model.
    """
    if not getattr(cli, "_app", None):
        ...
        cprint(f"\n{_DIM}  ✓ Stored secret in ~/.vulti/.env as {var_name}{_RST}")
        return {
            **stored,
            "skipped": False,
