...
1. **User plugins**   – ``~/.vulti/plugins/<name>/``
2. **Project plugins** – ``./.vulti/plugins/<name>/``
3. **Pip plugins**     – packages that expose the ``vulti_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "vulti_agent.plugins"

_NS_PARENT = "vulti_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.vulti/plugins/)
        vulti_home = os.environ.get("VULTI_HOME", os.path.expanduser("~/.vulti"))
        user_dir = Path(vulti_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.vulti/plugins/)
        project_dir = Path.cwd() / ".vulti" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``vulti_plugins.<name>``."""
