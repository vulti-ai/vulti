"""
Gateway subcommand for vulti CLI.

Handles: vulti gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from vulti_cli.config import get_env_value, get_vulti_home, save_env_value
from vulti_cli.setup import (
...
_SERVICE_BASE = "vulti-gateway"
SERVICE_DESCRIPTION = "Vulti Gateway - Messaging Platform Integration"


def get_service_name() -> str:
    """Derive a systemd service name scoped to this VULTI_HOME.

    Default ``~/.vulti`` returns ``vulti-gateway`` (backward compatible).
    Any other VULTI_HOME appends a short hash so multiple installations
    ...
    home = _Path(os.getenv("VULTI_HOME", _Path.home() / ".vulti")).resolve()
    default = (_Path.home() / ".vulti").resolve()
    if home == default:
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    vulti gateway uninstall")
    print_info("    sudo vulti gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Vulti can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo vulti gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo vulti gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo vulti gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "ai.vulti.gateway.plist"
...
def get_vulti_cli_path() -> str:
    """Get the path to the vulti CLI."""
    ...
    return f"{get_python_path()} -m vulti_cli.main"
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    vulti_home = str(Path(os.getenv("VULTI_HOME", Path.home() / ".vulti")).resolve())
...
User={username}
Group={group_name}
ExecStart={python_path} -m vulti_cli.main gateway run --replace
...
Environment="VULTI_HOME={vulti_home}"
Restart=on-failure
RestartSec=10
KillMode=mixed
KillSignal=SIGTERM
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Vulti install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}vulti gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}vulti gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}vulti gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}vulti gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}vulti gateway start{scope_flag}")
...
<dict>
    <key>Label</key>
    <string>ai.vulti.gateway</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>vulti_cli.main</string>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Vulti install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  vulti gateway status             # Check status")
    print("  tail -f ~/.vulti/logs/gateway.log  # View logs")
...
def launchd_start():
    ...
    plist_path = get_launchd_plist_path()
    try:
        subprocess.run(["launchctl", "start", "ai.vulti.gateway"], check=True)
...
def launchd_stop():
    subprocess.run(["launchctl", "stop", "ai.vulti.gateway"], check=True)
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Vulti install")
    else:
        print("⚠ Service definition is stale relative to the current Vulti install")
        print("  Run: vulti gateway start")
    ...
    if deep:
        log_file = get_vulti_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Vulti",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Vulti will use (e.g., vulti@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_vulti_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'vulti pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: vulti pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'vulti gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from vulti_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"VultiAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: vulti gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: vulti gateway")
    else:
        print()
        print_info("No platforms configured. Run 'vulti gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: vulti gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: vulti gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  vulti gateway install")
                print("  sudo vulti gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  vulti gateway          # Run in foreground")
                print("  vulti gateway install  # Install as user service")
                print("  sudo vulti gateway install --system  # Install as boot-time system service")
