"""
Interactive setup wizard for Vulti.
...
Config files are stored in ~/.vulti/ for easy access.
"""
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from vulti_cli.config import get_env_value
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🛡 Vulti Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Vulti using environment variables or config commands:")
    print_info("  vulti config set model.provider custom")
    print_info("  vulti config set model.base_url http://localhost:8080/v1")
    print_info("  vulti config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'vulti setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from vulti_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'vulti setup')")


def _print_setup_summary(config: dict, vulti_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'vulti setup' to configure"))
    ...
    if tts_provider == "elevenlabs" and get_env_value("ELEVENLABS_API_KEY"):
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'vulti setup tts'"))
    else:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'vulti setup tools' to configure them,"
        )
        print_warning("or edit ~/.vulti/.env directly to add the missing API keys.")
        print()
    ...
    print(color("📁 All your files are in ~/.vulti/:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('Settings:', Colors.YELLOW)}  {get_config_path()}")
    print(f"   {color('API Keys:', Colors.YELLOW)}  {get_env_path()}")
    print(
        f"   {color('Data:', Colors.YELLOW)}      {vulti_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('vulti setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('vulti setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('vulti setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('vulti setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('vulti setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('vulti config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('vulti config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('vulti config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('vulti', Colors.GREEN)}              Start chatting")
    print(f"   {color('vulti gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('vulti doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `vulti tools` and `vulti setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for Vulti to work.")
        print()
    ...
    if provider_idx == 0:  # Nous Portal (OAuth)
        ...
        try:
            from vulti_cli.auth import _login_nous, ProviderConfig
            import argparse
            ...
            print_info("You can try again later with: vulti model")
        ...
        try:
            from vulti_cli.auth import deactivate_provider
        ...
        config_path = (
            Path(os.environ.get("VULTI_HOME", Path.home() / ".vulti")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        if base_url:
            ...
            config_path = (
                Path(os.environ.get("VULTI_HOME", Path.home() / ".vulti"))
                / "config.yaml"
            )
            try:
                disk_cfg = {}
        ...
        if api_key:
            ...
            from vulti_cli.auth import detect_zai_endpoint
        ...
        from vulti_cli.auth import PROVIDER_REGISTRY
        from vulti_cli.config import save_anthropic_api_key, save_anthropic_oauth_token
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Vulti can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Vulti can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'vulti setup' or configure AUXILIARY_VISION_* settings")
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from vulti_cli.models import provider_model_ids
            ...
            from vulti_cli.models import model_ids, menu_labels
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'vulti setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Vulti runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Vulti via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("VULTI_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("VULTI_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Vulti from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Vulti delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://vulti-agent.nousresearch.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'vulti whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'vulti whatsapp' to choose your mode (separate bot number")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     vulti config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: vulti gateway install")
            else:
                print_info("  You can install later: vulti gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo vulti gateway install --system")
                print_info("  Or run in foreground:  vulti gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   vulti gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `vulti setup tools` and `vulti tools` use the same flow:
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_vulti.py"
)


def _offer_openclaw_migration(vulti_home: Path) -> bool:
    ...
    print_info("Vulti can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_vulti", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=vulti_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Vulti.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      vulti setup           — full or quick (auto-detected)
      vulti setup model     — just model/provider
      vulti setup terminal  — just terminal backend
      vulti setup gateway   — just messaging platforms
      vulti setup tools     — just tool configuration
      vulti setup agent     — just agent settings
    """
    ensure_vulti_home()
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     🛡 Vulti Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    from vulti_cli.auth import get_active_provider
    ...
    print(
        color(
            "│             🛡 Vulti Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Vulti installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Vulti configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, vulti_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'vulti setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'vulti setup' again when ready.")
            return
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, vulti_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        if _offer_openclaw_migration(vulti_home):
    ...
    print_info(f"Data folder:  {vulti_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'vulti config edit'")
...
def _run_quick_setup(config: dict, vulti_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'vulti setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Vulti to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'vulti setup gateway'.")
