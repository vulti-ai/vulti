#!/usr/bin/env python3
"""
Vulti CLI - Main entry point.

Usage:
    vulti                     # Interactive chat (default)
    vulti chat                # Interactive chat
    vulti gateway             # Run gateway in foreground
    vulti gateway start       # Start gateway as service
    vulti gateway stop        # Stop gateway service
    vulti gateway status      # Show gateway status
    vulti gateway install     # Install gateway service
    vulti gateway uninstall   # Uninstall gateway service
    vulti setup               # Interactive setup wizard
    vulti logout              # Clear stored authentication
    vulti status              # Show status of all components
    vulti cron                # Manage cron jobs
    vulti cron list           # List cron jobs
    vulti cron status         # Check if cron scheduler is running
    vulti doctor              # Check configuration and dependencies
    vulti honcho setup                    # Configure Honcho AI memory integration
    vulti honcho status                   # Show Honcho config and connection status
    vulti honcho sessions                 # List directory → session name mappings
    vulti honcho map <name>               # Map current directory to a session name
    vulti honcho peer                     # Show peer names and dialectic settings
    vulti honcho peer --user NAME         # Set user peer name
    vulti honcho peer --ai NAME           # Set AI peer name
    vulti honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    vulti honcho mode                     # Show current memory mode
    vulti honcho mode [hybrid|honcho|local]  # Set memory mode
    vulti honcho tokens                   # Show token budget settings
    vulti honcho tokens --context N       # Set session.context() token cap
    vulti honcho tokens --dialectic N     # Set dialectic result char cap
    vulti honcho identity                 # Show AI peer identity representation
    vulti honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    vulti honcho migrate                  # Step-by-step migration guide: OpenClaw native → Vulti + Honcho
    vulti version             Show version
    vulti update              Update to latest version
    vulti uninstall           Uninstall Vulti
    vulti acp                 Run as an ACP server for editor integration
    vulti sessions browse     Interactive session picker with search

    vulti claw migrate --dry-run  # Preview migration without changes
"""
...
# Load .env from ~/.vulti/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from vulti_cli.config import get_vulti_home
from vulti_cli.env_loader import load_vulti_dotenv
load_vulti_dotenv(project_env=PROJECT_ROOT / '.env')

# Point mini-swe-agent at ~/.vulti/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(get_vulti_home()))
...
from vulti_cli import __version__, __release_date__
from vulti_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from vulti_cli.config import get_env_path, get_vulti_home
    from vulti_cli.auth import get_auth_status

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from vulti_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_vulti_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'vulti sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Vulti isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  vulti setup")
        print()

        from vulti_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'vulti setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from vulti_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from vulti_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from vulti_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Vulti?")
        print()
    ...
    session_dir = get_vulti_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: vulti gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  vulti gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '🛡 Vulti'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  vulti gateway")
        ...
        print()
        print("  Or install as a service: vulti gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'vulti whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from vulti_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    from vulti_cli.config import load_config, save_config, get_env_value, save_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from vulti_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from vulti_cli.config import get_env_value, save_env_value
    ...
    from vulti_cli.models import model_ids
    ...
    if selected:
        ...
        from vulti_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from vulti_cli.codex_models import get_codex_model_ids
    from vulti_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from vulti_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from vulti_cli.auth import _save_model_choice, deactivate_provider
    from vulti_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from vulti_cli.models import probe_api_models
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `vulti model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from vulti_cli.auth import _save_model_choice, deactivate_provider
    from vulti_cli.config import save_env_value, load_config, save_config
    from vulti_cli.models import fetch_api_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print("    Vulti will use Claude's credential store directly instead of copying a setup-token into ~/.vulti/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from vulti_cli.models import _PROVIDER_MODELS
    ...
    if selected:
        ...
        # providers if the user switches without running 'vulti model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Vulti CLI with a provider."""
    from vulti_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from vulti_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from vulti_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from vulti_cli.cron import cron_command
    cron_command(args)
...
def cmd_config(args):
    ...
    from vulti_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Vulti v{__version__} ({__release_date__})")
    ...
    try:
        from vulti_cli.banner import check_for_updates
        behind = check_for_updates()
        if behind and behind > 0:
            commits_word = "commit" if behind == 1 else "commits"
            print(f"Update available: {behind} {commits_word} behind — run 'vulti update'")
        elif behind == 0:
            print("Up to date")
    except Exception:
        pass
...
def cmd_uninstall(args):
    """Uninstall Vulti."""
    from vulti_cli.uninstall import run_uninstall
    run_uninstall(args)


def _update_via_zip(args):
    """Update Vulti by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/vulti-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="vulti-update-")
        zip_path = os.path.join(tmp_dir, f"vulti-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to vulti-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"vulti-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("vulti-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Vulti behaves unexpectedly.")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Vulti couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Vulti couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Vulti behaves unexpectedly.")
    return True

def _invalidate_update_cache():
    """Delete the update-check cache so ``vulti --version`` doesn't
    report a stale "commits behind" count after a successful update."""
    try:
        cache_file = Path(os.getenv(
            "VULTI_HOME", Path.home() / ".vulti"
        )) / ".update_check"
...
def cmd_update(args):
    """Update Vulti to the latest version."""
    import shutil
    
    print("🛡 Updating Vulti...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/vulti-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'vulti config migrate' later to configure.")
        else:
        ...
        # Uses the PID file (scoped to VULTI_HOME) to find this
        # installation's gateway — safe with multiple installations.
        try:
            from gateway.status import get_running_pid, remove_pid_file
            from vulti_cli.gateway import (
            ...
            if is_macos():
                try:
                    ...
                    if plist_path.exists():
                        check = subprocess.run(
                            ["launchctl", "list", "ai.vulti.gateway"],
                            capture_output=True, text=True, timeout=5,
                        )
            ...
            if existing_pid or has_systemd_service or has_launchd_service:
                ...
                if has_systemd_service:
                    ...
                    if restart.returncode == 0:
                        ...
                        if is_linux():
                            ...
                            if linger_ok is not True:
                                ...
                                print("    vulti gateway restart")
                            else:
                                print("  Try manually: vulti gateway restart")
                    ...
                    print("  Restart it with: vulti gateway run")
        except Exception as e:
            logger.debug("Gateway restart during update failed: %s", e)
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  vulti model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``vulti -c Pokemon Agent Dev`` without quoting the
...
def main():
    """Main entry point for vulti CLI."""
    parser = argparse.ArgumentParser(
        prog="vulti",
        description="Vulti - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    vulti                        Start interactive chat
    vulti chat -q "Hello"        Single query mode
    vulti -c                     Resume the most recent session
    vulti -c "my project"        Resume a session by name (latest in lineage)
    vulti --resume <session_id>  Resume a specific session by ID
    vulti setup                  Run setup wizard
    vulti logout                 Clear stored authentication
    vulti model                  Select default model
    vulti config                 View configuration
    vulti config edit            Edit config in $EDITOR
    vulti config set model gpt-4 Set a config value
    vulti gateway                Run messaging gateway
    vulti -s vulti-agent-dev,github-auth
    vulti -w                     Start in isolated git worktree
    vulti gateway install        Install gateway background service
    vulti sessions list          List past sessions
    vulti sessions browse        Interactive session picker
    vulti sessions rename ID T   Rename/title a session
    vulti update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Vulti"
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Vulti with an interactive wizard. "
                    "Run a specific section: vulti setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Vulti CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: vulti-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Vulti components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Vulti setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Vulti configuration"
    )
    ...
    def cmd_pairing(args):
        from vulti_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from vulti_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from vulti_cli.skills_hub import skills_command
            skills_command(args)
    ...
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "each turn so Vulti knows who you are without you having to repeat yourself.\n\n"
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Vulti Honcho",
    )
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'vulti tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # vulti tools disable <name...> [--platform cli]
    ...
    # vulti tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from vulti_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            from vulti_cli.tools_config import tools_command
            tools_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch vulti --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            vulti_bin = shutil.which("vulti")
            if vulti_bin:
                os.execvp(vulti_bin, ["vulti", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "vulti_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Vulti"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Vulti",
    ...
    def cmd_claw(args):
        from vulti_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Vulti to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Vulti",
        description="Remove Vulti from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Vulti as an ACP (Agent Client Protocol) server",
        description="Start Vulti in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Vulti as an ACP server."""
        try:
    ...
    # e.g. ``vulti -c Pokemon Agent Dev`` → ``vulti -c 'Pokemon Agent Dev'``
