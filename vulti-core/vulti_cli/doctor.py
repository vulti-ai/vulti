"""
Doctor command for vulti CLI.

Diagnoses issues with Vulti setup.
"""
...
from vulti_cli.config import get_project_root, get_vulti_home, get_env_path

PROJECT_ROOT = get_project_root()
VULTI_HOME = get_vulti_home()

# Load environment variables from ~/.vulti/.env so API key checks work
...
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(VULTI_HOME))
os.environ.setdefault("MSWEA_SILENT_STARTUP", "1")

from vulti_cli.colors import Colors, color
from vulti_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.vulti/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `vulti`.
    os.environ.setdefault("VULTI_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Vulti Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.vulti/.env (primary location for user config)
    env_path = VULTI_HOME / '.env'
    if env_path.exists():
        check_ok("~/.vulti/.env file exists")
        ...
        if _has_provider_env_config(content):
            ...
            check_warn("No API key found in ~/.vulti/.env")
            issues.append("Run 'vulti setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.vulti/.env file missing")
            if should_fix:
                env_path.parent.mkdir(parents=True, exist_ok=True)
                env_path.touch()
                check_ok("Created empty ~/.vulti/.env")
                check_info("Run 'vulti setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'vulti setup' to create one")
                issues.append("Run 'vulti setup' to create .env")
    
    # Check ~/.vulti/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = VULTI_HOME / 'config.yaml'
    if config_path.exists():
        check_ok("~/.vulti/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.vulti/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.vulti/config.yaml manually")
            else:
    ...
    try:
        from vulti_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    vulti_home = VULTI_HOME
    if vulti_home.exists():
        check_ok("~/.vulti directory exists")
    else:
        if should_fix:
            vulti_home.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.vulti directory")
            fixed_count += 1
        else:
            check_warn("~/.vulti not found", "(will be created on first use)")
    
    # Check expected subdirectories
    expected_subdirs = ["cron", "sessions", "logs", "skills", "memories"]
    for subdir_name in expected_subdirs:
        subdir_path = vulti_home / subdir_name
        if subdir_path.exists():
            check_ok(f"~/.vulti/{subdir_name}/ exists")
        else:
            if should_fix:
                subdir_path.mkdir(parents=True, exist_ok=True)
                check_ok(f"Created ~/.vulti/{subdir_name}/")
                fixed_count += 1
            else:
                check_warn(f"~/.vulti/{subdir_name}/ not found", "(will be created on first use)")
    
    # Check for SOUL.md persona file
    soul_path = vulti_home / "SOUL.md"
    if soul_path.exists():
        content = soul_path.read_text(encoding="utf-8").strip()
        # Check if it's just the template comments (no real content)
        lines = [l for l in content.splitlines() if l.strip() and not l.strip().startswith(("<!--", "-->", "#"))]
        if lines:
            check_ok("~/.vulti/SOUL.md exists (persona configured)")
        else:
            check_info("~/.vulti/SOUL.md exists but is empty — edit it to customize personality")
    else:
        check_warn("~/.vulti/SOUL.md not found", "(create it to give Vulti a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Vulti Persona\n\n"
                "<!-- Edit this file to customize how Vulti communicates. -->\n\n"
                "You are Vulti, a helpful AI assistant.\n",
                encoding="utf-8",
            )
            check_ok("Created ~/.vulti/SOUL.md with basic template")
            fixed_count += 1
    
    # Check memory directory
    memories_dir = vulti_home / "memories"
    if memories_dir.exists():
        check_ok("~/.vulti/memories/ directory exists")
        ...
        check_warn("~/.vulti/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.vulti/memories/")
            fixed_count += 1
    
    # Check SQLite session store
    state_db_path = vulti_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.vulti/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.vulti/state.db exists but has issues: {e}")
    else:
        check_info("~/.vulti/state.db not created yet (will be created on first session)")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/vulti-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'vulti setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: vulti skills list)")
    ...
    if github_token:
        ...
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.vulti/.env for better rates)")
    ...
    try:
        ...
        if not GLOBAL_CONFIG_PATH.exists():
            check_warn("Honcho config not found", f"run: vulti honcho setup")
        elif not hcfg.enabled:
            check_info("Honcho disabled (set enabled: true in ~/.honcho/config.json to activate)")
        elif not hcfg.api_key:
            check_fail("Honcho API key not set", "run: vulti honcho setup")
            issues.append("No Honcho API key — run 'vulti honcho setup'")
        else:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'vulti doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
