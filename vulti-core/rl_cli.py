...
if tinker_atropos_dir.exists():
    ...
    os.environ['VULTI_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to vulti-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.vulti/config.yaml if not provided)
    ...
    """
    # Load config from ~/.vulti/config.yaml
    config = load_vulti_config()
