                        ...
                        LazyVStack(alignment: .leading, spacing: 8) {
                            ...
                            if ws.isStreaming && !ws.streamingContent.isEmpty {
                                HStack(alignment: .top, spacing: 8) {
                                    ThinkingAvatar()
                                    MarkdownMessageView(
                                        content: stripToolCallXML(ws.streamingContent),
                                        isUser: false
                                    )
        ...
        .onAppear {
            ...
            if sessionId == nil && !app.introspectedAgents.contains(agentId) {
                ...
                Task {
                    ...
                    if let todaySession = sessions.first(where: { isSessionFresh($0) }) {
...
/// Strip tool call XML from message content so it does not render as raw text.
private func stripToolCallXML(_ text: String) -> String {
    var result = text
    let patterns = [
        "<invoke[\\s\\S]*?</invoke>",
        "<function_calls>[\\s\\S]*?</function_calls>",
        "<antml:invoke[\\s\\S]*?</antml:invoke>",
    ]
...
struct MessageBubble: View {
    ...
    var body: some View {
        ...
        HStack {
            ...
            MarkdownMessageView(
                content: stripToolCallXML(message.content ?? ""),
                isUser: isUser
            )
                .padding(.horizontal, 14)
                .padding(.vertical, 10)
